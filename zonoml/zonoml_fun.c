...
elina_abstract0_t *ffn_add_bias_zono(elina_manager_t *man, bool destructive,
                                     elina_abstract0_t *element,
                                     elina_dim_t start_offset, double *bias,
                                     size_t num_var) {
  start_timing();
  ...
  elina_linexpr0_t **expr_array =
      (elina_linexpr0_t **)malloc(num_var * sizeof(elina_linexpr0_t *));
  elina_dim_t *tdim_array =
      (elina_dim_t *)malloc(num_var * sizeof(elina_dim_t));
  ...
  res = zonotope_assign_linexpr_array(man, true, res, tdim_array, expr_array,
                                      num_var, NULL);
  for (i = 0; i < num_var; i++) {
...
elina_abstract0_t *conv_matmult_zono(
  ...
  conv_matmult_zono_parallel(
      pr, res, start_offset, filter_weights, filter_bias, num_out_neurons,
