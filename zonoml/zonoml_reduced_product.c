		...
		if(sup<=0){
			...
			elina_double_interval_div(&lambda_l, &lambda_u, sup_l, sup_u, width_l, width_u);
                        ...
                        if ((-inf > sup) && -bound_l > 1) {
                          ...
                          double alpha_l, alpha_u;
                          elina_double_interval_div(&alpha_l, &alpha_u, -1, 1,
                                                    bound_l, bound_u);
                          ...
                          double dev_sup = 0.0;
                          zonotope_aff_t *res_box = zonotope_aff_alloc_init(pr);
                          elina_interval_middev(&mid_inf, &mid_sup, &dev_inf,
                                                &dev_sup, 0, sup_u);
                          res_box->c_inf = res_box->c_inf + mid_inf;
                          res_box->c_sup = res_box->c_sup + mid_sup;
                          res_box->itv_inf = 0.0;
                          ...
                          elina_interval_set_double(lambda, -lambda_l,
                                                    lambda_u);
                          zonotope_aff_t *res_zono =
                              zonotope_aff_mul_itv(pr, zo->paf[offset], lambda);
                          zonotope_aff_check_free(pr, zo->paf[offset]);
                          ...
                          zonotope_aff_t *res =
                              zonotope_aff_add(pr, res_box, res_zono, zo);
                          zo->paf[offset] = res;
...
void *handle_maxpool_zono_parallel(void *args) {
  ...
  for (mat_x = idx_start; mat_x < idx_end; mat_x++) {
    ...
    size_t inp_z = out_z;
    // long int inp_pos = (inp_x*strides[0]-pad_top)*i12 +
    // (inp_y*strides[1]-pad_left)*input_size[2] + inp_z; size_t pool_start_dim =
    // mat_x*pool_size[0]*pool_size[1]; printf("inpXYZ: %zu, %zu, %zu\n", inp_x,
    // inp_y, inp_z); printf("outXYZ: %zu, %zu, %zu\n", out_x, out_y, out_z);
    ...
    for (x_shift = 0; x_shift < pool_size[0]; x_shift++) {
      for (y_shift = 0; y_shift < pool_size[1]; y_shift++) {
        ...
        elina_interval_t *bound =
            zonotope_bound_dimension(pr->man, z, pool_cur_dim);
    ...
    if (flag) {
      ...
      double dev_sup = 0.0;
      zonotope_aff_t *res_box = zonotope_aff_alloc_init(pr);
      elina_interval_middev(&mid_inf, &mid_sup, &dev_inf, &dev_sup, -max_l,
                            max_u);
      res_box->c_inf = res_box->c_inf + mid_inf;
      res_box->c_sup = res_box->c_sup + mid_sup;
...
elina_abstract0_t *maxpool_zono(elina_manager_t *man, bool destructive,
  ...
  // fflush(stdout);
  maxpool_zono_parallel(pr, res, src_offset, pool_size, num_out_neurons,
                        dst_offset, input_size, strides, output_size, pad_top,
                        pad_left, handle_maxpool_zono_parallel);
