...
#ifdef QSORT_EXTRA_CMP_ARGUMENT
void opt_qsort2(void *base_ptr, size_t count, size_t size, opt_qsort2_cmp cmp,
	    void *cmp_argument)
#else
void opt_qsort(void *base_ptr, size_t count, size_t size, opt_qsort_cmp cmp)
#endif
{
  char *low, *high, *pivot;
  opt_stack_node stack[STACK_SIZE], *stack_ptr;
  char ptr_cmp;
