...
void opt_poly_widening_gen(elina_manager_t *man, opt_pk_array_t *op,
  ...
  ********************************/
  unsigned short int *rmapa =
      (unsigned short int *)calloc(num_compa, sizeof(unsigned short int));
  ...
  *********************************/
  unsigned short int *rmapb =
      (unsigned short int *)calloc(num_compb, sizeof(unsigned short int));
  ...
  **************************************/
  unsigned short int **ca_arr =
      (unsigned short int **)malloc(num_comp * sizeof(unsigned short int *));
  unsigned short int *comp_size_arr =
      (unsigned short int *)calloc(num_comp, sizeof(unsigned short int));
  ...
  // opt_satmat_t ** satF_arr = (opt_satmat_t
  // **)malloc(num_comp*sizeof(opt_satmat_t*));
  ...
  for (k = 0; k < num_comp; k++) {
    ...
    tmp[k]->F =
        opt_matrix_alloc(nbgena[k] + 2 * num_vertex[k], comp_size + 2, false);
    poly[k]->C =
        opt_matrix_alloc(opk->dec - 1 + nbmapb[k], comp_size + 2, false);
  ...
  // unsigned short int ** ind_map_a_arr = (unsigned short int
  // **)malloc(num_compa*sizeof(unsigned short int *)); cla = acla->head;
  ...
  //	if(opt_vector_is_positivity_constraint(opk,
  //pci,poly_a[ka]->C->nbcolumns)){ 		continue;
  //	}
  ...
  for (kb = 0; kb < num_compb; kb++) {
    ...
    *********************/
    ...
    for (i = 0; i < nbcons; i++) {
      ...
      opt_numint_t *npci = opt_map_vector(pci, ind_map_b, comp_size_arr[indb],
                                          obk->C->nbcolumns);
      // size_t start = 0;
      ...
      if (index >= 0) {
        ...
        // !opt_vector_is_positivity_constraint(opk,
        // oak->C->p[index],oak->C->nbcolumns)){
