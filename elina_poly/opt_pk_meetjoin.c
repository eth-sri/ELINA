	...
	*************************/
        if (!opk->exn) {
          ...
          if ((max > 0) && (max_finest_cl == NULL)) {
            ...
            // poly[0]->F =
            // opt_matrix_alloc(F->nbrows,max_finest_cl->size+opk->dec,false);
            ...
            // bool is_pos1 = false;
            // poly[0]->nbline =
            // split_matrix(opk,poly[0]->F,F,ind_map1,max_finest_cl->size,
            // &is_pos1); size_t lines_removed =
            ...
            // poly[0]->satC =
            // opt_satmat_alloc(poly[0]->F->nbrows,opt_bitindex_size(poly[0]->C->nbrows));
            ...
            if (rem_size) {
              ...
              // poly[num_comp_res]->F =
              // opt_matrix_alloc(F->nbrows,rem_size+opk->dec,false); bool
              // is_pos1 = false; poly[num_comp_res]->nbline =
              // split_matrix(opk,poly[num_comp_res]->F,F,ind_map2,rem_size,
              // &is_pos1); lines_removed =
              // remove_common_gen(opk,poly[num_comp_res]->F,0);
              // poly[num_comp_res]->nbline = poly[num_comp_res]->nbline -
              // lines_removed;
              // remove_positivity_constraint(opk,poly[num_comp_res]->C);
              // opt_matrix_sort_rows(opk,poly[num_comp_res]->F);
              // poly[num_comp_res]->satC =
              // opt_satmat_alloc(poly[num_comp_res]->F->nbrows,opt_bitindex_size(poly[num_comp_res]->C->nbrows));
