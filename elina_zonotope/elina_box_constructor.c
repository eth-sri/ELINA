				 ...
				 elina_box_t* a, elina_linexpr0_t* expr)
{
  ...
  if (a->p==NULL){
    ...
    exact = elina_interval_eval_elina_linexpr0(interval,expr,a->p,ELINA_SCALAR_DOUBLE);
...
elina_lincons0_array_t elina_box_to_lincons_array(elina_manager_t* man, elina_box_t* a)
{
    ...
    if (a->p==NULL){
        ...
        size = 0;
        for (i=0;i<nbdims;i++){
            if (!elina_scalar_infty(a->p[i]->inf)) size++;
            bool point = !elina_scalar_infty(a->p[i]->inf) && elina_scalar_equal(a->p[i]->inf, a->p[i]->sup);
            if (!point && !elina_scalar_infty(a->p[i]->sup)) size++;
        }
        ...
        for (i=0;i<nbdims;i++){
            ...
            if (!elina_scalar_infty(a->p[i]->inf)){
                ...
                point = !elina_scalar_infty(a->p[i]->inf) && elina_scalar_equal(a->p[i]->inf, a->p[i]->sup);
