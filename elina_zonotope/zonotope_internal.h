...
static inline void zonotope_noise_symbol_fprint(FILE *stream,
                                                zonotope_noise_symbol_t *eps) {
  switch (eps->type) {
  case IN:
    fprintf(stream, "(eps%u)", eps->index);
    break;
  case UN:
    fprintf(stream, "(eta%u)", eps->index);
    break;
  default:
    fprintf(stderr, "error: unknown type of noise symbol, aborting...\n");
    abort();
    break;
  }
}
...
static inline void zonotope_aaterm_fprint(zonotope_internal_t *pr, FILE *stream,
                                          zonotope_aaterm_t *ptr) {
...
static inline void zonotope_aff_fprint(zonotope_internal_t *pr, FILE *stream,
                                       zonotope_aff_t *expr) {
  if (expr == NULL) {
    return;
  }
