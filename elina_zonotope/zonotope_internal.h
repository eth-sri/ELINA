    ...
    /* increment the global index of used noise symbols and add the noise symbol in pr->eps */
{
    ...
    {
      if (!elina_scalar_infty(ptr->coeff->inf) &&
          elina_scalar_equal(ptr->coeff->inf, ptr->coeff->sup)) {
        elina_scalar_fprint(stream, ptr->coeff->sup);
      } else
        elina_interval_fprint(stream, ptr->coeff);
      fprintf(stream, ".");
        ...
        if ((!elina_scalar_sgn(lambda) )|| zonotope_aff_is_known_to_be_zero(pr, src)) {
          return zonotope_aff_alloc_init(pr);
            ...
            if (src->q) {
                ...
                for (p=src->q; p; p=p->n) {
                    if(sgn>=0){
                      elina_scalar_mul(q->coeff->inf, p->coeff->inf, lambda,
                                       ELINA_SCALAR_DOUBLE);
                      elina_scalar_mul(q->coeff->sup, p->coeff->sup, lambda,
                                       ELINA_SCALAR_DOUBLE);
                    }
                    else{
                        //elina_scalar_t * add = elina_scalar_alloc_set(tmp->sup);
                        elina_scalar_mul(q->coeff->sup, p->coeff->inf, lambda,
                                         ELINA_SCALAR_DOUBLE);
                        elina_scalar_mul(q->coeff->inf, p->coeff->sup, lambda,
                                         ELINA_SCALAR_DOUBLE);
                        //elina_scalar_free(add);
                    }
                    // elina_interval_mul(q->coeff, lambda,
                    // p->coeff,ELINA_SCALAR_DOUBLE);
...
static inline void zonotope_aff_cons_eq_lambda(zonotope_internal_t* pr, elina_interval_t** res, zonotope_aff_t* x, zonotope_aff_t* cons, zonotope_t *z)
{
	...
	if (p && q) {
	    if (p->pnsym->index == q->pnsym->index) {
              ...
              if (elina_scalar_sgn(p->coeff->inf) ||
                  elina_scalar_sgn(p->coeff->sup)) {
                ...
                if (z->hypercube) {
                  ...
                  if (zonotope_noise_symbol_cons_get_dimpos(
                          pr, &dim, p->pnsym->index, z)) {
                    elina_interval_set(dim_itv, z->gamma[dim]);
                    elina_interval_middev(mid, dev, dim_itv);
                    elina_interval_abs(tmp, p->coeff, ELINA_SCALAR_DOUBLE);
                    elina_interval_mul(array[i]->coeff, tmp, dev,
                                       ELINA_SCALAR_DOUBLE);
                  } else {
                    elina_interval_abs(tmp, p->coeff, ELINA_SCALAR_DOUBLE);
                    elina_interval_set(array[i]->coeff, tmp);
                ...
                elina_interval_div(tmp, q->coeff, p->coeff,
                                   ELINA_SCALAR_DOUBLE);
                if (!elina_scalar_infty(tmp->inf) &&
                    elina_scalar_equal(tmp->inf, tmp->sup))
                  elina_interval_neg(array[i]->itv, tmp);
                // else {
                // bound_neg(tmp->inf, tmp->sup);
                // elina_interval_set_num(array[i]->itv,
                // bound_numref(tmp->inf));
                //}
                i++;
              }
                p = p->n ;
...
static inline bool zonotope_aff_reduce(zonotope_internal_t* pr, zonotope_aff_t *expr)
{
    ...
    if (elina_scalar_infty(expr->c->inf) || elina_scalar_infty(expr->c->sup)) {
      ...
      for (p = expr->q; p; p = p->n) {
        if (elina_scalar_infty(p->coeff->inf) ||
            elina_scalar_infty(p->coeff->sup)) {
          ...
          elina_scalar_sub(err, p->coeff->sup, p->coeff->inf,
                           ELINA_SCALAR_DOUBLE);
          if (elina_scalar_cmp(err, eps) > 0) {
...
static inline bool zonotope_aff_is_eq(zonotope_internal_t* pr, zonotope_aff_t *z1, zonotope_aff_t *z2)
{
	...
	for (p=z1->q, q=z2->q; p && q;) {
	    if (p->pnsym != q->pnsym) return false;
            else if (!elina_interval_equal(p->coeff, q->coeff))
              return false;
            else {
		p = p->n;
		q = q->n;
        ...
        if (p && q) {
          if (p->pnsym->index == q->pnsym->index) {
            ...
            if (p->pnsym->type == UN) {
              ...
              if (s == -1) {
                elina_interval_sub(qmptr, q->coeff, ptr->coeff,
                                   ELINA_SCALAR_DOUBLE);
                // elina_interval_set_int(pmptr, 0,0);
              } else if (s == 1) {
                // elina_interval_set_int(qmptr, 0,0);
                elina_interval_sub(pmptr, p->coeff, ptr->coeff,
                                   ELINA_SCALAR_DOUBLE);
              } else {
