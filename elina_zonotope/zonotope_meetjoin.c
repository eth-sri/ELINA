    ...
    /* TODO destructive not used  */
{
	...
	if (z1->hypercube && z2->hypercube) {
		...
		if (dims1 > res->size) {
		    res->nsymcons = (elina_dim_t*)realloc(res->nsymcons, (dims1)*sizeof(elina_dim_t));
		    res->gamma = (elina_interval_t**)realloc(res->gamma, (dims1)*sizeof(elina_interval_t*));
		...
		elina_abstract0_add_dimensions(pr->manNS, true, z2->abs, dimchange2, false);
		elina_dimchange_add_invert(dimchange2);
		for (k=0; k<dim2; k++) {
		    zonotope_set_lincons_dim(pr, dimchange2->dim[k]);
		    elina_abstract0_meet_lincons_array(pr->manNS, true, z2->abs, &pr->moo);
		}
		...
		dimchange2->realdim = dims2; elina_dimchange_free(dimchange2);
