    ...
    /* TODO destructive not used  */
{
	...
	if ((z1->hypercube && z2->hypercube)) {
		    ...
		    res->paf[i] = z1->paf[i];
			 ...
			 res->paf[i] = pr->top;
                  ...
                  if (elina_scalar_infty(z1->box[i]->inf) ||
                    ...
                    /* Do nothing, the join of concretisations is already done
                     * and stored in res->box */
                    res->paf[i] = zonotope_aff_alloc_init(pr);
                    elina_interval_set(res->paf[i]->c, res->box[i]);
                  } else {
		...
		elina_abstract0_t * tmp_z2 = elina_abstract0_add_dimensions(pr->manNS, false, z2->abs, dimchange2, false);
                elina_abstract0_t *tmp_z1 = elina_abstract0_add_dimensions(
                    pr->manNS, false, z1->abs, dimchange1, false);
		...
		for (k=0; k<dim2; k++) {
		    ...
		    elina_abstract0_meet_lincons_array(pr->manNS, true, tmp_z2, &pr->moo);
                ...
                for (k = 0; k < dim1; k++) {
                  ...
                  elina_abstract0_meet_lincons_array(pr->manNS, true, tmp_z1,
                                                     &pr->moo);
                }
                elina_dimension_t jdim1 =
                    elina_abstract0_dimension(pr->manNS, tmp_z1);
                elina_dimension_t jdim2 =
                    elina_abstract0_dimension(pr->manNS, tmp_z2);

                res->abs =
                    elina_abstract0_join(pr->manNS, false, tmp_z1, tmp_z2);
