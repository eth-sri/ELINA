	...
	if ((z1->hypercube && z2->hypercube)) {
		...
		for (k=0; k<dim2; k++) {
		    ...
		    elina_abstract0_meet_lincons_array(pr->manNS, true, tmp_z2, &pr->moo);
                ...
                // elina_abstract0_dimension(pr->manNS,res->abs);
                // elina_dimension_t jdim2 =
                // elina_abstract0_dimension(pr->manNS,tmp_z2); printf("INPUTS %d
		...
		zonotope_update_noise_symbol_cons_gamma(pr, res);
                ...
                // elina_abstract0_fprint(stdout,pr->manNS,res->abs,NULL);
                // elina_dimension_t jdim1 =
                // elina_abstract0_dimension(pr->manNS,res->abs); for(k=0; k <
                // zonotope_noise_symbol_cons_get_dimension(pr, res); k++){
                //	printf("%u ",res->nsymcons[k]);
                //}
                // printf("\n");
                // fflush(stdout);
