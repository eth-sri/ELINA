...
void precompute_for_tanh(const int K,
        ...
        for (size_t vi = 0; vi < num_v; vi++) {
            ...
            for (int xi = 0; xi < K; xi++) {
                ...
                for (size_t i = base; i < base + num; i++) {
                    ...
                    mpq_add(first[xi + 1 + K], first[xi + 1 + K], b_arr[quadrant[xi]][xi]);

                    mpq_mul(second[xi + 1 + K], second[xi + 1], k_arr[quadrant[xi]][xi]);
                }
                assert(V.size() - base == 2 * num && "The number of new vertices should've doubled.");
            }
        }
