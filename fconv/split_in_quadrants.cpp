...
vector<Adj> compute_adjacency_from_incidence(const vector<set_t>& incidence) {
    size_t n = incidence.size();

    vector<Adj> potential_adjacencies;
    vector<set_t> adjacencies_incidence;
    potential_adjacencies.reserve(n * (n - 1) / 2);
    adjacencies_incidence.reserve(n * (n - 1) / 2);
    for (size_t i = 0; i < n; i++) {
        for (size_t j = i + 1; j < n; j++) {
            // A potential optimization here is analogue of Chernikova criteria.
            // Pair can create an adjacency, only if it's incident to more than a
            // certain number of constraints.
            potential_adjacencies.emplace_back(i, j);
            adjacencies_incidence.push_back(set_intersect(incidence[i], incidence[j]));
        }
    }
...
void split_in_quadrants_recursive(Quadrant &quadrant,
                                  const vector<Adj> &adjacencies,
                                  // Incidence here is V to H.
                                  vector<set_t> &incidence,
                                  vector<mpq_t *> &vrep,
                                  map<Quadrant, QuadrantInfo> &quadrant2info,
                                  const int K, const int NUM_H) {
  const int xi = (int)quadrant.size();
  assert(incidence.size() == vrep.size() &&
         "The size of incidence should equal the size of vrep.");
  ...
  vector<int> V_to_sign(num_v,
                        0); // -1 means xi < 0; 0 means xi == 0; 1 means xi > 0
  ...
  for (size_t vi = 0; vi < num_v; vi++) {
    ...
    if (sign == 0) {
      V_zero.push_back(vi);
      V_to_sign[vi] = 0;
      // It's okay to modify incidence because from it I will be creating two
      // copies that will be passed down to children recursively.
  ...
  // adj_split is a list of adjacencies that are split by this orthant -
  // i.e. one vertex of the adjacency is on one side, and another vertex is on
  // the another side.
  vector<Adj> adj_split;
  vector<Adj> adj_minus;
  vector<Adj> adj_plus;
  // The procedure might lead to creating the duplicate adjacencies between
  // existing vertices that are incident to the new orthant. In order to avoid
  // this problem, I record the adjacencies between such vertices to ensure
  // there is no duplication.
  set<Adj> adj_zero;
  for (const auto &adj : adjacencies) {
    assert(
        (0 <= adj.first && adj.first < adj.second && adj.second < (int)num_v) &&
    ...
    if (sign1 <= 0 && sign2 <= 0) {
      ...
      assert(new_first != -1 && new_second != -1 &&
             "Consistency check for generating minus adj.");
    ...
    if (sign1 >= 0 && sign2 >= 0) {
      ...
      assert(new_first != -1 && new_second != -1 &&
             "Consistency check for generating plus adj.");
  ...
  // Because I would need to compute adjacency between newly created vertices
  // and vertices that are incident to the new orthant, I create a list of
  // incidences of these vertices.
  vector<set_t> incidence_of_V_new_and_zero(num_new_v + V_zero.size());
  ...
  for (int adj_i = 0; adj_i < (int)adj_split.size(); adj_i++) {
    ...
    incidence_of_V_new_and_zero[adj_i] =
        set_intersect(incidence[adj.first], incidence[adj.second]);
    ...
    assert(mpq_sgn(v[xi + 1]) == 0 &&
           "The cur element of the new vertex have to be 0.");
    assert(mpq_cmp_si(v[0], 1, 1) == 0 &&
           "First element of the new vertex have to be 1.");

    if (a_sign < 0) {
      // adj.first -> in negative branch, adj.second -> in positive.
      // count_minus + adj_i -> will be a shifted index of a generated point.
      assert(V_to_minus[adj.first] != -1 &&
             "Consistency check when creating new adj for minus.");
      assert(V_to_plus[adj.second] != -1 &&
             "Consistency check when creating new adj for plus.");
      ...
      assert(V_to_minus[adj.second] != -1 &&
             "Consistency check when creating new adj for minus.");
      assert(V_to_plus[adj.first] != -1 &&
             "Consistency check when creating new adj for plus.");
  ...
  vector<Adj> new_adjacencies =
      compute_adjacency_from_incidence(incidence_of_V_new_and_zero);
  ...
  for (const auto &adj : new_adjacencies) {
    ...
    if (adj.first >= (int)num_new_v && adj.second >= (int)num_new_v) {
      // If both vertices of the adjacency are old vertices that are incident to
      // the new orthant I need to check if I have already enumerated their
      // adjacency. If that's the case, adjacency has to be skipped to avoid
      // duplication.
    ...
    for (int i = xi + 1; i < K; i++) {
      ...
      if (sign_h1 != 0 && sign_h2 != 0 && sign_h1 != sign_h2) {
        // Adjacency is useful only if it is to be split by one of the future
        // orthants.
        useful = true;
        break;
      }
  ...
  vector<set_t> incidence_minus(count_minus + num_new_v);
  vector<set_t> incidence_plus(count_plus + num_new_v);
  for (size_t vi = 0; vi < num_v; vi++) {
    ...
    if (sign >= 0) {
      ...
      if (sign > 0) {
        // If vertex goes exclusively to the plus branch - then plus branch
        // assumes ownership. And no copy happens.
...
split_in_quadrants(vector<mpq_t *> &V, vector<set_t> &incidence,
  ...
  ASRTF(V.size() == incidence.size(),
        "Number of vertexes should equal number of incidences.");

  const int NUM_H = set_size(incidence[0]);
  for (size_t i = 0; i < incidence.size(); i++) {
    set_resize(incidence[i], NUM_H + K);
  }

  Quadrant quadrant{};
  quadrant.reserve(K);
  map<Quadrant, QuadrantInfo> quadrant2info;
  split_in_quadrants_recursive(quadrant, orthant_adjacencies, incidence, V,
                               quadrant2info, K, NUM_H);
