...
vector<Adj> compute_adjacency_from_incidence(const vector<bset> &incidence) {
  ...
  for (size_t i = 0; i < n; i++) {
    for (size_t j = i + 1; j < n; j++) {
      ...
      // Pair can create an adjacency, only if it's incident more to a certain
...
void split_in_quadrants_recursive(Quadrant &quadrant,
  ...
  // adj_split is a list of adjacencies that are split by this orthant - i.e.
  // one vertex of the adjacency is on one side, and another vertex is on the
  // another side.
  ...
  vector<bset> incidence_of_V_new_and_zero(num_new_v + V_zero.size(),
                                           bset(NUM_H + K));
  ...
  for (int adj_i = 0; adj_i < (int)adj_split.size(); adj_i++) {
    ...
    incidence_of_V_new_and_zero[adj_i] =
        incidence[adj.first] & incidence[adj.second];
