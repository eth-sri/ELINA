...
double line_s_curve_soundness_margin(double x, double k, double b,
...
double min_margin_concave_region(double k, double b, double x_lb, double x_ub,
                                 bool is_upper, bool is_sigm) {
  ...
  double v_left =
      line_s_curve_soundness_margin(x_left, k, b, is_upper, is_sigm);
  double v_right =
      line_s_curve_soundness_margin(x_right, k, b, is_upper, is_sigm);
  ...
  while (x_right - x_left > EPS) {
    ...
    double v_left_third =
        line_s_curve_soundness_margin(x_left_third, k, b, is_upper, is_sigm);
    double v_right_third =
        line_s_curve_soundness_margin(x_right_third, k, b, is_upper, is_sigm);

    ASRTF((v_left_third <= v_left + EPS / 20 ||
...
double min_margin_convex_region_and_zero(double k, double b, double x_lb,
                                         double x_ub, bool is_upper,
                                         bool is_sigm) {
...
void test_S_curve_bounds(double lb, double ub, int N_samples, bool is_sigm) {
  ...
  cout << "running S curve bounds test lb " << lb << " ub " << ub << " N "
       << N_samples << " for " << activation << endl;
  ...
  for (int i = 0; i < N_samples; i++) {
    ...
    double m_lb_concave =
        min_margin_concave_region(k_lb, b_lb, x_lb, x_ub, false, is_sigm);
    double m_lb_convex = min_margin_convex_region_and_zero(
        k_lb, b_lb, x_lb, x_ub, false, is_sigm);
    double m_ub_concave =
        min_margin_concave_region(k_ub, b_ub, x_lb, x_ub, true, is_sigm);
    double m_ub_convex = min_margin_convex_region_and_zero(k_ub, b_ub, x_lb,
                                                           x_ub, true, is_sigm);
    double margins[] = {m_lb_concave, m_lb_convex, m_ub_concave, m_ub_convex};
    double min_margin = *min_element(margins, margins + 4);
    ASRTF(min_margin >= EPS, "Failed x_lb " + to_string(x_lb) + " x_ub " +
                                 to_string(x_ub) + " margin " +
                                 to_string(min_margin));
  }
  cout << "\tpassed" << endl;
}
