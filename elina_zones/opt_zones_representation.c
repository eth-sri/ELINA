...
elina_dimension_t opt_zones_dimension(elina_manager_t* man, opt_zones_t* o)
{
  ...
  opt_zones_internal_t* pr = opt_zones_init_from_manager(man,ELINA_FUNID_DIMENSION,0);
  elina_dimension_t r;
  r.intdim = o->intdim;
...
{
  opt_zones_internal_t* pr = opt_zones_init_from_manager(man,ELINA_FUNID_ASIZE,0);
  if (!o->m) return 1;
