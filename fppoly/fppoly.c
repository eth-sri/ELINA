...
void ffn_handle_first_layer(elina_manager_t* man, elina_abstract0_t * abs, double **weights, double *bias,  size_t size, size_t num_pixels, activation_type_t activation){
	...
	size_t i, j;
	//printf("Interval %p %p %p\n",res->layers, res->input_inf, res->input_sup);
	//for(i=0; i < num_pixels; i++){
	...
	for(i=0; i < size; i++){
		...
		for(j=0; j < num_pixels; j++){
			//double inf = itv[j]->inf->val.dbl;
			//double sup = itv[j]->sup->val.dbl;
			//if(weight_i[j]<0){
			//	ub = ub + weight_i[j]*inf;	
			//	lb = lb - weight_i[j]*sup;
			//}
			//else{
			//	ub = ub + weight_i[j]*sup;
			//	lb = lb - weight_i[j]*inf;
			//}
			double tmp1 = 0.0;
			double tmp2 = 0.0;
			elina_double_interval_mul_cst_coeff(pr, &tmp1,&tmp2,-weight_i[j],weight_i[j],res->input_inf[j],res->input_sup[j]);
			lb = lb + tmp1;
			ub = ub + tmp2;
		}
...
expr_t * lexpr_replace_maxpool_bounds(fppoly_internal_t * pr, expr_t * expr, neuron_t ** neurons){
	...
	if(expr->sup_coeff[0]<0){
		...
		if(neuron_k->maxpool_uexpr==NULL){
			...
			res = multiply_expr(pr,neuron_k->maxpool_uexpr,expr->inf_coeff[0],expr->sup_coeff[0]);
		}
		...
		if(neuron_k->maxpool_lexpr==NULL){
			...
			res = multiply_expr(pr,neuron_k->maxpool_lexpr,expr->inf_coeff[0],expr->sup_coeff[0]);
		}
		//printf("multiply end %zu \n",k);
		//expr_print(res);
		//fflush(stdout);
	...
	for(i = 1; i < num_neurons; i++){
		...
		if(expr->sup_coeff[i]<0){
			...
			if(neuron_k->maxpool_uexpr==NULL){
				...
				mul_expr = multiply_expr(pr,neuron_k->maxpool_uexpr,expr->inf_coeff[i],expr->sup_coeff[i]);
			...
			if(neuron_k->maxpool_lexpr==NULL){
				...
				mul_expr = multiply_expr(pr,neuron_k->maxpool_lexpr,expr->inf_coeff[i],expr->sup_coeff[i]);
...
expr_t * uexpr_replace_maxpool_bounds(fppoly_internal_t * pr, expr_t * expr, neuron_t ** neurons){
	size_t num_neurons = expr->size;
	size_t i,k;
	expr_t *res;
	if(expr->type==DENSE){
	...
	for(i = 1; i < num_neurons; i++){
		...
		neuron_t *neuron_k = neurons[k];
		if(expr->sup_coeff[i]<0){
			expr_t * mul_expr = multiply_expr(pr,neuron_k->maxpool_lexpr,expr->inf_coeff[i],expr->sup_coeff[i]);
			add_expr(pr,res,mul_expr);
			free_expr(mul_expr);
		}
		else if (expr->inf_coeff[i]<0){
			expr_t * mul_expr = multiply_expr(pr,neuron_k->maxpool_uexpr,expr->inf_coeff[i],expr->sup_coeff[i]);
			add_expr(pr,res,mul_expr);
			free_expr(mul_expr);
		}
		else{
...
double compute_lb_from_expr(expr_t * expr, fppoly_t * fp){
	...
	for(i=0; i < dims; i++){
		...
		if(expr->type==DENSE){
			...
			//res = res + expr->coeff[i]*fp->input_inf[i];	
		//}
		//else if(expr->sup_coeff[i]<0){
		//	elina_double_interval_mul(&tmp1,&tmp2,expr->inf_coeff[i],expr->sup_coeff[i],-fp->input_sup[i],fp->input_sup[i]);
		//	res_inf = res_inf + tmp1;
		//	res_sup = res_sup + tmp2;
			//res = res - expr->coeff[i]*fp->input_sup[i];
		//}
		//else{
		//}
...
bool is_greater(elina_manager_t* man, elina_abstract0_t* element, elina_dim_t y, elina_dim_t x){
	...
	if(exprA==NULL){
		...
		if(exprB==NULL){
			...
			//expr_print(sub);
			//fflush(stdout);
			double lb = compute_lb_from_expr(sub,fp);
	...
	for(out_x=0; out_x < output_size[0]; out_x++) {
	    for(out_y = 0; out_y < output_size[1]; out_y++) {
		 for(out_z=0; out_z < output_size[2]; out_z++) {
		   ...
		   // expr_print(neurons[mat_x]->expr);
		    double lb = 0;
		    double ub = 0;
		    lb = -cst;
		    ub = cst;
		    for(j=0; j < actual_coeff; j++){
			double tmp1 = 0.0;
			double tmp2 = 0.0;
			elina_double_interval_mul_cst_coeff(pr,&tmp1,&tmp2,-coeff[j],coeff[j],res->input_inf[dim[j]],res->input_sup[dim[j]]);
			lb = lb + tmp1;
			ub = ub + tmp2;
		   }
