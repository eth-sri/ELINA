...
void handle_concatenation_layer(elina_manager_t *man,
  ...
  // printf("FC start here %zu %zu %zu
  // %zu\n",num_in_neurons,num_out_neurons,predecessors[0],num_predecessors);
  // fflush(stdout);
  fppoly_t *fp = fppoly_of_abstract0(element);
  size_t numlayers = fp->numlayers;
  // printf("concat_layer: %zu, first inout: %zu, last input: %zu\n", numlayers,
  // predecessors[0], predecessors[num_predecessors - 1]); fflush(stdout);
  ...
  for (i = 0; i < num_predecessors; i++) {
    ...
    for (j = 0; j < num_in_neurons; j++) {
      ...
      // if(k==1){
      //	printf("TRUE BOUNDS: %zu
      //%g\n",k,fp->layers[pred-1]->neurons[j]->lb); 	fflush(stdout);
      //	}
