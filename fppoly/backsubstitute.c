...
void update_state_using_predecessor_layer(fppoly_internal_t *pr, fppoly_t *fp,
  ...
  *lexpr_ptr = lexpr_replace_bounds(pr, lexpr, aux_neurons,
                                    fp->layers[k]->is_activation);
  *uexpr_ptr = uexpr_replace_bounds(pr, uexpr, aux_neurons,
                                    fp->layers[k]->is_activation);
  free_expr(tmp_l);
  free_expr(tmp_u);
}
                ...
                while (k >= 0) {
                  ...
                  if (fp->layers[k]->num_predecessors == 2) {
                    ...
                    while (iter != common_predecessor) {
                      ...
                      update_state_using_predecessor_layer(pr, fp, &lexpr,
                                                           &uexpr, iter);
		...
		size_t idx_end = idx_start + idx_n;
	  	...
	  	for (i = 0; i < NUM_THREADS; i++){
			...
			if((i==NUM_THREADS-2)){
                ...
                // NULL,update_state_using_previous_layers, (void*)&args[i]);
