...
__device__ void
lcoeff_replace_relu_bounds(float_type &inf_coeff, float_type &sup_coeff,
  ...
  if ((old_sup_coeff == 0) && (old_inf_coeff == 0)) {
    ...
    interval_mul_expr_coeff(inf_coeff, sup_coeff, lambda_inf, lambda_sup,
                            old_inf_coeff, old_sup_coeff);
    float_type tmp1, tmp2;
    interval_mul_cst_coeff(tmp1, tmp2, mu_inf, mu_sup, old_inf_coeff,
                           old_sup_coeff);
...
__device__ void
ucoeff_replace_relu_bounds(float_type &inf_coeff, float_type &sup_coeff,
                           float_type &res_inf_cst, float_type &res_sup_cst,
                           const float_type lb, const float_type ub,
                           const bool use_area_heuristic) {
  const float_type width_inf = add_rd(ub, -lb);
