...
void update_state_using_previous_layers_residual_chunk(
    elina_manager_t *man, fppoly_t *fp, const size_t layerno,
    const size_t num_chunks, const size_t chunk_counter,
    const bool use_area_heuristic) {
  auto start = std::chrono::system_clock::now();
    ...
    if (fp->layers[k]->type == RESIDUAL) {
      if (fp->layers[k]->activation == RELU) {
        lexpr_replace_relu_bounds_conv_sparse<<<
            ...
            1>>>(linf_coeff, lsup_coeff, linf_cst, lsup_cst,
                 fp->layers[k]->lb_array, fp->layers[k]->ub_array,
                 fp->layers[k - 1]->output_size[0],
                 fp->layers[k - 1]->output_size[1],
                 fp->layers[k - 1]->output_size[2], offset_x, offset_y,
                 length_x, length_y, shift_x, shift_y, use_area_heuristic);
        uexpr_replace_relu_bounds_conv_sparse<<<
            dim3(fp->layers[layerno]->output_size[0],
                 fp->layers[layerno]->output_size[1],
                 fp->layers[layerno]->output_size[2] / num_chunks),
      ...
      const size_t num_filters_current_layer =
          fp->layers[predecessor1]->output_size[2];
...
void handle_residual_layer(elina_manager_t *man, elina_abstract0_t *element,
  ...
  update_state_using_previous_layers_residual(man, fp, fp->numlayers - 1,
                                              use_area_heuristic);
}
