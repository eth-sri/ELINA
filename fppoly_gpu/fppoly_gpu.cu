                ...
                for(size_t inp_x = 0; inp_x < input_size_x; inp_x++)
                {
                    for(size_t inp_y = 0; inp_y < input_size_y; inp_y++)
                    {
                        for(size_t inp_z = 0; inp_z < input_size_z; inp_z++)
                        {
                            ...
                            const size_t c = i*input_size_x*input_size_y*input_size_z + j;
...
void update_state_using_previous_layers(elina_manager_t* man, fppoly_t* fp, const size_t layerno)
{
    ...
    if(fp->layers[layerno]->type == CONV)
    {
        ...
        std::cout << "offset_y " << offset_y << " offset_x " << offset_x << std::endl;

        length_y = y_max_0 - y_min_0;
        length_x = x_max_0 - x_min_0;

        std::cout << "length_y " << length_y << " length_x " << length_x << std::endl;
    ...
    for(int k = layerno - 1; k >= 0; k--)
    {
        ...
        if(fp->layers[k]->type == CONV)
        {
            ...
            {
                coeffs_from_previous_layer_conv<<<num_out_neurons_last_layer, 1>>>(linf_coeff, lsup_coeff, linf_coeff_tmp, lsup_coeff_tmp, aux_coeffs, fp->layers[k]->output_size[0], fp->layers[k]->output_size[1], fp->layers[k]->output_size[2], fp->layers[k]->input_size[0], fp->layers[k]->input_size[1], fp->layers[k]->input_size[2], fp->layers[k]->filter_size[0], fp->layers[k]->filter_size[1], fp->layers[k]->strides[0], fp->layers[k]->strides[1], fp->layers[k]->pad[0], fp->layers[k]->pad[1]);
                coeffs_from_previous_layer_conv<<<num_out_neurons_last_layer, 1>>>(uinf_coeff, usup_coeff, uinf_coeff_tmp, usup_coeff_tmp, aux_coeffs, fp->layers[k]->output_size[0], fp->layers[k]->output_size[1], fp->layers[k]->output_size[2], fp->layers[k]->input_size[0], fp->layers[k]->input_size[1], fp->layers[k]->input_size[2], fp->layers[k]->filter_size[0], fp->layers[k]->filter_size[1], fp->layers[k]->strides[0], fp->layers[k]->strides[1], fp->layers[k]->pad[0], fp->layers[k]->pad[1]);
            }
