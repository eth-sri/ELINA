...
__global__ void compute_lb_from_expr(double *lb_array, double **inf_coeff,
                                     double **sup_coeff, const double *inf_cst,
                                     const double *input_inf,
                                     const double *input_sup,
  ...
  if (n < num_exprs) {
    ...
    for (size_t i = 0; i < expr_size; i++) {
      elina_double_interval_mul(&tmp1, &tmp2, inf_coeff[n][i], sup_coeff[n][i],
...
__global__ void compute_ub_from_expr(double *ub_array, double **inf_coeff,
                                     double **sup_coeff, const double *sup_cst,
                                     const double *input_inf,
                                     const double *input_sup,
                                     const size_t num_exprs,
                                     const size_t expr_size) {
...
__global__ void device_layer_create_dense_expr(
    double **inf_coeff, double **sup_coeff, double *inf_cst, double *sup_cst,
    const double *weights, const double *bias, const size_t num_out_neurons,
...
void layer_create_dense_exprs(double **inf_coeff, double **sup_coeff,
...
__global__ void
copy_expr_array(double **target_inf_coeff, double **target_sup_coeff,
                double *target_inf_cst, double *target_sup_cst,
                double **source_inf_coeff, double **source_sup_coeff,
                double *source_inf_cst, double *source_sup_cst,
...
__global__ void free_coeff_arrays(double **inf_coeff, double **sup_coeff,
                                  const size_t size) {
  size_t i = blockIdx.x;
...
void layer_compute_bounds_from_exprs(double **inf_coeff, double **sup_coeff,
                                     double *inf_cst, double *sup_cst,
                                     double *lb_array, double *ub_array,
...
__global__ void lexpr_replace_relu_bounds(
    double **inf_coeff, double **sup_coeff, double *inf_cst, double *sup_cst,
    double *lb_array, double *ub_array, const size_t num_out_neurons_last_layer,
  ...
  if (n < num_out_neurons_last_layer) {
    if (i < num_out_neurons_current_layer) {
      ...
      if ((old_sup_coeff == 0) && (old_inf_coeff == 0)) {
        ...
        elina_double_interval_mul_expr_coeff(&inf_coeff[n][i], &sup_coeff[n][i],
        ...
        if ((area1 < area2) && (area1 < area3)) {
          elina_double_interval_mul_expr_coeff(
              &inf_coeff[n][i], &sup_coeff[n][i], lambda_inf, lambda_sup,
              old_inf_coeff, old_sup_coeff);
...
__global__ void expr_from_previous_layer(
    double **expr_inf_coeff, double **expr_sup_coeff, double *expr_inf_cst,
    double *expr_sup_cst, double **res_inf_coeff, double **res_sup_coeff,
    double *res_inf_cst, double *res_sup_cst, double **aux_inf_coeff,
    double **aux_sup_coeff, double *aux_inf_cst, double *aux_sup_cst,
    ...
    if (j < num_in_neurons_current_layer) {
      if (j == 0) {
        ...
        elina_double_interval_mul_cst_coeff(
            &res_inf_cst[n], &res_sup_cst[n], expr_inf_coeff[n][0],
            expr_sup_coeff[n][0], aux_inf_cst[i], aux_sup_cst[i]);
        ...
        for (i = 1; i < num_out_neurons_current_layer; i++) {
          if ((expr_inf_coeff[n][i] != 0) || (expr_sup_coeff[n][i] != 0)) {
            elina_double_interval_mul_cst_coeff(
                &tmp1, &tmp2, expr_inf_coeff[n][i], expr_sup_coeff[n][i],
                aux_inf_cst[i], aux_sup_cst[i]);
      ...
      elina_double_interval_mul_expr_coeff(
          &res_inf_coeff[n][j], &res_sup_coeff[n][j], expr_inf_coeff[n][0],
          expr_sup_coeff[n][0], aux_inf_coeff[i][j], aux_sup_coeff[i][j]);
      ...
      for (i = 1; i < num_out_neurons_current_layer; i++) {
        if ((expr_inf_coeff[n][i] != 0) || (expr_sup_coeff[n][i] != 0)) {
          elina_double_interval_mul_expr_coeff(
              &tmp1, &tmp2, expr_inf_coeff[n][i], expr_sup_coeff[n][i],
              aux_inf_coeff[i][j], aux_sup_coeff[i][j]);

          maxRes = fmax(fabs(res_inf_coeff[n][j]), fabs(res_sup_coeff[n][j]));
          maxMul = fmax(fabs(tmp1), fabs(tmp2));

          res_inf_coeff[n][j] =
              res_inf_coeff[n][j] + tmp1 + (maxRes + maxMul) * ulp;
          res_sup_coeff[n][j] =
              res_sup_coeff[n][j] + tmp2 + (maxRes + maxMul) * ulp;
        }
      }
    }
  }
...
__global__ void layer_allocate_exprs(double **inf_coeff, double **sup_coeff,
                                     double *inf_cst, double *sup_cst,
...
void update_state_using_previous_layers(elina_manager_t *man, fppoly_t *fp,
  ...
  cudaMalloc((void **)&linf_coeff,
             num_out_neurons_last_layer * sizeof(double *));
  ...
  for (int k = layerno - 1; k >= 0; k--) {
    ...
    layer_allocate_exprs<<<num_out_neurons_last_layer, 1>>>(
        uinf_coeff_tmp, usup_coeff_tmp, uinf_cst_tmp, usup_cst_tmp,
        num_out_neurons_last_layer, num_in_neurons_current_layer);

    expr_from_previous_layer<<<num_blocks_linear, num_threads>>>(
        linf_coeff, lsup_coeff, linf_cst, lsup_cst, linf_coeff_tmp,
        lsup_coeff_tmp, linf_cst_tmp, lsup_cst_tmp, aux_inf_coeff,
        aux_sup_coeff, aux_inf_cst, aux_sup_cst, num_out_neurons_last_layer,
        num_out_neurons_current_layer, num_in_neurons_current_layer);
    expr_from_previous_layer<<<num_blocks_linear, num_threads>>>(
        uinf_coeff, usup_coeff, uinf_cst, usup_cst, uinf_coeff_tmp,
        usup_coeff_tmp, uinf_cst_tmp, usup_cst_tmp, aux_inf_coeff,
        aux_sup_coeff, aux_inf_cst, aux_sup_cst, num_out_neurons_last_layer,
        num_out_neurons_current_layer, num_in_neurons_current_layer);
    ...
    free_coeff_arrays<<<num_out_neurons_last_layer, 1>>>(
        linf_coeff_tmp, lsup_coeff_tmp, num_out_neurons_last_layer);
    free_coeff_arrays<<<num_out_neurons_last_layer, 1>>>(
        uinf_coeff_tmp, usup_coeff_tmp, num_out_neurons_last_layer);
  }
  ...
  free_coeff_arrays<<<num_out_neurons_last_layer, 1>>>(
      linf_coeff, lsup_coeff, num_out_neurons_last_layer);
  free_coeff_arrays<<<num_out_neurons_last_layer, 1>>>(
      uinf_coeff, usup_coeff, num_out_neurons_last_layer);
...
__global__ void print_bounds(double *bounds_array,
                             const size_t num_out_neurons) {
  for (size_t i = 0; i < num_out_neurons; i++) {
...
__global__ void create_sub_expr(double **inf_coeff, double **sup_coeff,
                                double *inf_cst, double *sup_cst,
...
void device_layer_create_sparse_exprs(
    ...
    const size_t *filter_size, const size_t *strides, const long int pad_top,
    const long int pad_left, const size_t num_pixels) {
  const size_t num_out_neurons =
...
void free_layer(layer_t *layer) {
  free_coeff_arrays<<<layer->num_out_neurons, 1>>>(
      layer->inf_coeff, layer->sup_coeff, layer->num_out_neurons);
