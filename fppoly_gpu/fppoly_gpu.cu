...
uexpr_dim, size_t expr_size){ fppoly_t * res = (fppoly_t
*)malloc(sizeof(fppoly_t)); fppoly_from_network_input_box(res, intdim, realdim,
inf_array, sup_array); size_t num_pixels = intdim + realdim; res->input_lexpr =
(expr_t **)malloc(num_pixels*sizeof(expr_t *)); res->input_uexpr = (expr_t
**)malloc(num_pixels*sizeof(expr_t *));
        ...
        for(i = 0; i < num_pixels; i++){
                ...
                res->input_lexpr[i] = create_sparse_expr(tmp_weights,
lexpr_cst[i], tmp_dim, expr_size); sort_sparse_expr(res->input_lexpr[i]);
...
lexpr_dim[i][2]);
                ...
                res->input_uexpr[i] = create_sparse_expr(tmp_weights,
uexpr_cst[i], tmp_dim, expr_size); sort_sparse_expr(res->input_uexpr[i]);
...
= expr->sup_cst; res->type = expr->type; res->size = num_neurons;
        ...
        for(i = 0; i < num_neurons; i++){
                ...
                if(is_sigmoid){
                        ...
                        den_inf_l = -1 + e_inf_l;
                        den_inf_u = 1 + e_inf_u;
                        elina_double_interval_div(&f_sup_l, &f_sup_u, e_sup_l,
e_sup_u, den_sup_l, den_sup_u); elina_double_interval_div(&f_inf_l, &f_inf_u,
e_inf_l, e_inf_u, den_inf_l, den_inf_u);
                }
                else{
                        f_inf_l = e_inf_l;
                        f_inf_u = e_inf_u;
                    ...
                    res->sup_cst = res->sup_cst - tmp1;
                        ...
                        if(expr->sup_coeff[i] < 0){
                                if(ub<0){
                                        ...
                                        compute_chord_slope(&slope_inf,
&slope_sup, f_sup_l, f_sup_u, f_inf_l, f_inf_u, lb, -lb, -ub, ub);
                                        //if(slope_inf>0){
                                        //boxify=true;
                                        //}
                                        //else{
                                        //compute_derivative( &slope_inf,
&slope_sup, e_inf_l, e_inf_u, den_inf_l, den_inf_u, is_sigmoid);
                                        //printf("slope2: %.30f
...
e_sup_l, e_sup_u, den_sup_l, den_sup_u, is_sigmoid); x_l = ub; x_u = -ub; f_x_l
...
add_sup, tmp1, tmp2); if(-tmp3<f_inf_u){
                                ...
                                if(lb<=ub){
                                        ...
                                        compute_derivative( &slope_inf,
&slope_sup, e_sup_l, e_sup_u, den_sup_l, den_sup_u, is_sigmoid);
...
add_sup, tmp1, tmp2); if(tmp4>-f_sup_l){
                    ...
                    if(boxify){
                        ...
                        res->sup_cst = res->sup_cst - tmp1;
                                ...
                                elina_double_interval_mul_cst_coeff(pr,
&mul_inf, &mul_sup, add_inf, add_sup, expr->inf_coeff[i], expr->sup_coeff[i] );
...
expr_t *lexpr_replace_maxpool_bounds(fppoly_internal_t *pr, expr_t *expr,
                                     neuron_t **neurons) {
  // printf("begin\n");
  // fflush(stdout);
  size_t num_neurons = expr->size;
...
expr_t *uexpr_replace_maxpool_bounds(fppoly_internal_t *pr, expr_t *expr,
                                     neuron_t **neurons) {
  size_t num_neurons = expr->size;
  size_t i, k;
  expr_t *res;
...
bool is_greater(elina_manager_t *man, elina_abstract0_t *element, elina_dim_t y,
  ...
  if(exprA==NULL){
          ...
          if(exprB==NULL){
                  ...
                  if(exprA->type==DENSE){
                          sub->inf_coeff =
  (double*)malloc(sizeA*sizeof(double)); sub->sup_coeff =
  (double*)malloc(sizeA*sizeof(double)); sub->dim = NULL; sub->size = sizeA;
                          sub->type = DENSE;
                          if(exprB->type==DENSE){
                                          for(i=0; i < sizeA; i++){
                                                  sub->inf_coeff[i] =
  exprA->inf_coeff[i] + exprB->sup_coeff[i]; sub->sup_coeff[i] =
  exprA->sup_coeff[i] + exprB->inf_coeff[i];
                                          }
                          }
                          else{
                                  k = 0;
                                  for(i=0; i < sizeA; i++){
                                          if(k < sizeB && exprB->dim[k]==i){
                                                  sub->inf_coeff[i] =
  exprA->inf_coeff[i] + exprB->sup_coeff[k]; sub->sup_coeff[i] =
  exprA->sup_coeff[i] + exprB->inf_coeff[k]; k++;
                                          }
                                          else{
                                                  sub->inf_coeff[i] =
  exprA->inf_coeff[i]; sub->sup_coeff[i] = exprA->sup_coeff[i];
                                          }
                                  }
                          }
                          ...
                          if(exprB->type==DENSE){
                                  sub->inf_coeff =
  (double*)malloc(sizeB*sizeof(double)); sub->sup_coeff =
  (double*)malloc(sizeB*sizeof(double)); sub->dim = NULL; sub->size = sizeB;
                                  sub->type = DENSE;
                                  i = 0;
                                  for(k=0; k < sizeB; k++){
                                          if(i < sizeA && exprA->dim[i]==k){
                                                  sub->inf_coeff[k] =
  exprA->inf_coeff[i] + exprB->sup_coeff[k]; sub->sup_coeff[k] =
  exprA->sup_coeff[i] + exprB->inf_coeff[k]; i++;
                                          }
                                          else{
                                                  sub->inf_coeff[i] =
  exprB->sup_coeff[k]; sub->sup_coeff[i] = exprB->inf_coeff[k];
                                          }
                                  }
                          }
                          else{
  ...
  (double*)malloc((sizeA+sizeB)*sizeof(double)); sub->dim = NULL;
                                  ...
                                  k=0;
                                  sub->dim = (size_t
  *)malloc((sizeA+sizeB)*sizeof(size_t)); while(i < sizeA && k < sizeB){
                                          if(exprA->dim[i] < exprB->dim[k]){
                                                  sub->inf_coeff[l] =
  exprA->inf_coeff[i]; sub->sup_coeff[l] = exprA->sup_coeff[i]; sub->dim[l] =
  exprA->dim[i]; i++;

                                          }
                                          else if(exprB->dim[k] <
  exprA->dim[i]){ sub->inf_coeff[l] = exprB->sup_coeff[k]; sub->sup_coeff[l] =
  exprB->inf_coeff[k]; sub->dim[l] = exprB->dim[k]; k++;
                                          }
                                          else{
                                                  sub->inf_coeff[l] =
  exprA->inf_coeff[i] + exprB->sup_coeff[k]; sub->sup_coeff[l] =
  exprA->sup_coeff[i] + exprB->inf_coeff[k]; sub->dim[l] = exprA->dim[i]; i++;
                                                  k++;
                                          }
                                          l++;
                                  }
  ...
  exprA->dim[i]; i++; l++;
                                  ...
                                  while(k < sizeB){
                                          sub->inf_coeff[l] =
  exprB->inf_coeff[k]; sub->sup_coeff[l] = exprB->sup_coeff[k]; sub->dim[l] =
  exprB->dim[k]; k++; l++;
                                  }
                                  sub->size = l;
                                  sub->inf_coeff =
...
filter_size[1]+1) / (double)strides[1]);
                ...
                output_size[0] = ceil((double)input_size[0] /
(double)strides[0]); output_size[1] = ceil((double)input_size[1] /
(double)strides[1]);
        }
...
%zu\n",pool_cur_dim,fp->layers[numlayers-1]->dims,numlayers);
                                ...
                                double lb =
-fp->layers[numlayers-1]->neurons[pool_cur_dim]->lb; double ub =
fp->layers[numlayers-1]->neurons[pool_cur_dim]->ub; if(ub<=0){ inf[l] = 0.0;
                                        sup[l] = 0.0;
                                }
                                else if(lb>0){
                                        inf[l] = lb;
...
elina_interval_t **box_for_layer(elina_abstract0_t *abs, size_t layerno) {
  ...
  elina_interval_t **itv_arr =
      (elina_interval_t **)malloc(dims * sizeof(elina_interval_t *));
  size_t i;
  for (i = 0; i < dims; i++) {
