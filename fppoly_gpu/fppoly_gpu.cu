...
__device__
void elina_double_interval_mul_const_c(float_type* const a_inf, float_type* const a_sup, const float_type b_inf, const float_type b_sup, const float_type c)
{
    if(c >= 0)
    {
        *a_inf = b_inf*c;
...
void fppoly_from_network_input_box(fppoly_t* const res, const size_t intdim, const size_t realdim, const double* inf_array, const double* sup_array)
{
    ...
    cudaMemcpy(res->input_inf, tmp_input_inf, num_pixels*sizeof(float_type), cudaMemcpyHostToDevice);
    cudaMemcpy(res->input_sup, tmp_input_sup, num_pixels*sizeof(float_type), cudaMemcpyHostToDevice);
...
elina_abstract0_t* fppoly_from_network_input_poly(elina_manager_t* man, const size_t intdim, const size_t realdim, const double* inf_array, const double* sup_array, const double* lexpr_weights, const double* lexpr_cst, const size_t* lexpr_dim, const double* uexpr_weights, const double* uexpr_cst, const size_t* uexpr_dim, const size_t expr_size)
{
    ...
    cudaMalloc((void**) &(res->input_lweights), num_pixels*expr_size*sizeof(float_type));
    cudaMalloc((void**) &(res->input_uweights), num_pixels*expr_size*sizeof(float_type));

    cudaMalloc((void**) &(res->input_lcst), num_pixels*sizeof(float_type));
    cudaMalloc((void**) &(res->input_ucst), num_pixels*sizeof(float_type));

    cudaMemcpy(res->input_lweights, input_lweights_tmp, num_pixels*expr_size*sizeof(float_type), cudaMemcpyHostToDevice);
    cudaMemcpy(res->input_uweights, input_uweights_tmp, num_pixels*expr_size*sizeof(float_type), cudaMemcpyHostToDevice);

    cudaMemcpy(res->input_lcst, input_lcst_tmp, num_pixels*sizeof(float_type), cudaMemcpyHostToDevice);
    cudaMemcpy(res->input_ucst, input_ucst_tmp, num_pixels*sizeof(float_type), cudaMemcpyHostToDevice);
...
void ffn_add_layer(fppoly_t* const fp, const size_t num_out_neurons, const size_t num_in_neurons, const layertype_t type, const activation_type_t activation)
{
    ...
    cudaMalloc((void**) &layer->coeffs, num_out_neurons*num_in_neurons*sizeof(float_type));
...
__device__
void elina_double_interval_mul_expr_coeff(float_type* const res_inf, float_type* const res_sup, const float_type inf, const float_type sup, const float_type inf_expr, const float_type sup_expr)
{
...
__device__
void elina_double_interval_mul_expr_coeff_const_expr(float_type* const res_inf, float_type* const res_sup, const float_type inf, const float_type sup, const float_type expr)
{
    elina_double_interval_mul_const_c(res_inf, res_sup, inf, sup, expr);

    float_type tmp1, tmp2;

    elina_double_interval_mul_symmetric_c(&tmp1, &tmp2, inf, sup, abs(expr)*ulp);
...
__device__
void elina_double_interval_mul_cst_coeff_const_expr(float_type* const res_inf, float_type* const res_sup, const float_type inf, const float_type sup, const float_type expr)
{
    elina_double_interval_mul_expr_coeff_const_expr(res_inf, res_sup, inf, sup, expr);
...
__global__
void compute_lb_from_expr(float_type* __restrict__ lb_array, const float_type* __restrict__ inf_coeffs, const float_type* __restrict__ sup_coeffs, const float_type* __restrict__ inf_csts, const float_type* __restrict__ input_inf, const float_type* __restrict__ input_sup, const size_t expr_size)
{
    ...
    for(size_t i = 0; i < expr_size; i++)
    {
        ...
        {
            elina_double_interval_mul2(&tmp1, &tmp2, inf_coeff, sup_coeff, input_inf[i], input_sup[i]);
...
__global__
void compute_ub_from_expr(float_type* __restrict__ ub_array, const float_type* __restrict__ inf_coeffs, const float_type* __restrict__ sup_coeffs, const float_type* __restrict__ sup_csts, const float_type* __restrict__ input_inf, const float_type* __restrict__ input_sup, const size_t expr_size)
{
    const size_t n = blockIdx.x;
...
__global__
void compute_lb_from_expr_conv_sparse(float_type* __restrict__ lb_array, const float_type* __restrict__ inf_coeffs, const float_type* __restrict__ sup_coeffs, const float_type* __restrict__ inf_csts, const float_type* __restrict__ input_inf, const float_type* __restrict__ input_sup, const size_t num_chunks, const size_t chunk_counter, const size_t output_size_x, const size_t output_size_y, const size_t output_size_z, long int offset_x, long int offset_y, long int length_x, long int length_y, long int shift_x, long int shift_y)
{
    ...
    for(long int out_x = min_x; out_x < max_x; out_x++)
    {
        for(long int out_y = min_y; out_y < max_y; out_y++)
        {
            for(size_t out_z = 0; out_z < output_size_z; out_z++)
            {
                ...
                size_t j = out_x*length_y*output_size_z + out_y*output_size_z + out_z;
...
__global__
void device_layer_create_dense_expr(float_type* __restrict__ coeffs, float_type* __restrict__ csts, const FloatType* __restrict__ weights, const FloatType* __restrict__ bias, const size_t num_out_neurons, const size_t num_in_neurons)
{
...
void layer_create_dense_exprs(float_type* coeffs, float_type* csts, const double** weights, const double* bias, const size_t num_out_neurons, const size_t num_in_neurons)
{
    double* tmp_weights;
...
__global__
void copy_coeffs_and_csts(float_type* __restrict__ target_coeff, float_type* __restrict__ target_cst, const float_type* __restrict__ source_coeffs, const float_type* __restrict__ source_csts, const size_t coeff_size)
{
    const size_t i = blockIdx.x;
...
void add_coeffs_and_csts(float_type* __restrict__ target_inf_coeff, float_type* __restrict__ target_sup_coeff, float_type* __restrict__ target_inf_cst, float_type* __restrict__ target_sup_cst, const float_type* __restrict__ source_inf_coeff, const float_type* __restrict__ source_sup_coeff, const float_type* __restrict__ source_inf_cst, const float_type* __restrict__ source_sup_cst, const size_t expr_size)
{
    ...
    for(size_t j = 0; j < expr_size; j++)
    {
        ...
        target_inf_coeff[i*expr_size + j] = tar_inf + src_inf - (maxRes + maxMul)*ulp;
        target_sup_coeff[i*expr_size + j] = tar_sup + src_sup + (maxRes + maxMul)*ulp;
    }

    maxRes = max(abs(target_inf_cst[i]), abs(target_sup_cst[i]));
    maxMul = max(abs(source_inf_cst[i]), abs(source_sup_cst[i]));

    target_inf_cst[i] += source_inf_cst[i] - (maxRes + maxMul)*ulp - min_denormal;
    target_sup_cst[i] += source_sup_cst[i] + (maxRes + maxMul)*ulp + min_denormal;
}


__global__
void add_coeffs_and_csts_sparse(float_type* __restrict__ target_inf_coeff, float_type* __restrict__ target_sup_coeff, float_type* __restrict__ target_inf_cst, float_type* __restrict__ target_sup_cst, const float_type* __restrict__ source_inf_coeff, const float_type* __restrict__ source_sup_coeff, const float_type* __restrict__ source_inf_cst, const float_type* __restrict__ source_sup_cst, const size_t output_size_target_x, const size_t output_size_target_y, const size_t output_size_source_x, const size_t output_size_source_y, const size_t output_size_z, const size_t relative_offset_x, const size_t relative_offset_y)
{
    size_t out_x = blockIdx.x;
    size_t out_y = blockIdx.y;
    size_t out_z = blockIdx.z;

    const size_t n = out_x*gridDim.y*gridDim.z + out_y*gridDim.z + out_z;

    const size_t num_coeffs_target = output_size_target_x*output_size_target_y*output_size_z;
    const size_t num_coeffs_source = output_size_source_x*output_size_source_y*output_size_z;

    float_type maxRes;
    float_type maxMul;

    for(size_t out_x = 0; out_x < output_size_source_x; out_x++)
    {
        for(size_t out_y = 0; out_y < output_size_source_y; out_y++)
        {
            for(size_t out_z = 0; out_z < output_size_z; out_z++)
            {
                const size_t target_idx = (out_x + relative_offset_x)*output_size_target_y*output_size_z + (out_y + relative_offset_y)*output_size_z + out_z;

                const size_t source_idx = out_x*output_size_source_y*output_size_z + out_y*output_size_z + out_z;

                const float_type src_inf = source_inf_coeff[n*num_coeffs_source + source_idx];
                const float_type src_sup = source_sup_coeff[n*num_coeffs_source + source_idx];

                const float_type tar_inf = target_inf_coeff[n*num_coeffs_target + target_idx];
                const float_type tar_sup = target_sup_coeff[n*num_coeffs_target + target_idx];

                maxRes = max(abs(tar_inf), abs(tar_sup));
                maxMul = max(abs(src_inf), abs(src_sup));

                target_inf_coeff[n*num_coeffs_target + target_idx] = tar_inf + src_inf - (maxRes + maxMul)*ulp;
                target_sup_coeff[n*num_coeffs_target + target_idx] = tar_sup + src_sup + (maxRes + maxMul)*ulp;
            }
        }
    }
    ...
    target_inf_cst[n] += source_inf_cst[n] - (maxRes + maxMul)*ulp - min_denormal;
    target_sup_cst[n] += source_sup_cst[n] + (maxRes + maxMul)*ulp + min_denormal;
}
    ...
    for(size_t x_shift = 0; x_shift < filter_size_x; x_shift++)
    {
        for(size_t y_shift = 0; y_shift < filter_size_y; y_shift++)
        {
            for(size_t inp_z = 0; inp_z < input_size_z; inp_z++)
            {
                ...
                elina_double_interval_mul2(&tmp1, &tmp2, coeffs[filter_index], coeffs[filter_index], input_inf[mat_in], input_sup[mat_in]);
...
void layer_compute_bounds_from_exprs(const float_type* __restrict__ coeffs, const float_type* __restrict__ csts, float_type* lb_array, float_type* ub_array, const float_type* __restrict__ input_inf, const float_type* __restrict__ input_sup, const size_t num_out_neurons, const size_t num_in_neurons)
{
    ...
    {
        elina_double_interval_mul2(&tmp1, &tmp2, coeffs[n*num_in_neurons + i], coeffs[n*num_in_neurons + i], input_inf[i], input_sup[i]);
...
void lcoeffs_from_input_poly(const float_type* __restrict__ expr_inf_coeff, const float_type* __restrict__ expr_sup_coeff, float_type* __restrict__ res_inf_coeff, float_type* __restrict__ res_sup_coeff, const float_type* __restrict__ input_lcoeffs, const float_type* __restrict__ input_ucoeffs, const size_t num_out_neurons_current_layer, const size_t num_in_neurons_current_layer)
{
    ...
    while(j < num_in_neurons_current_layer)
    {
        ...
        for(size_t i = 0; i < num_out_neurons_current_layer; i++)
        {
            ...
            if(prev_inf_coeff > 0)
            {
                elina_double_interval_mul_expr_coeff_const_expr(&tmp1, &tmp2, prev_inf_coeff, prev_sup_coeff, input_lcoeffs[c]);
            ...
            if(prev_sup_coeff < 0)
            {
                elina_double_interval_mul_expr_coeff_const_expr(&tmp1, &tmp2, prev_inf_coeff, prev_sup_coeff, input_ucoeffs[c]);
...
void lcsts_from_input_poly(const float_type* __restrict__ expr_inf_coeff, const float_type* __restrict__ expr_sup_coeff, const float_type* __restrict__ expr_inf_cst, const float_type* __restrict__ expr_sup_cst, float_type* __restrict__ res_inf_cst, float_type* __restrict__ res_sup_cst, const float_type* __restrict__ input_lcsts, const float_type* __restrict__ input_ucsts, const float_type* __restrict__ input_inf, const float_type* __restrict__ input_sup, const size_t num_out_neurons_current_layer)
{
    ...
    for(size_t i = 0; i < num_out_neurons_current_layer; i++)
    {
        ...
        if(prev_inf_coeff > 0)
        {
            elina_double_interval_mul_cst_coeff_const_expr(&tmp1, &tmp2, prev_inf_coeff, prev_sup_coeff, input_lcsts[i]);
        ...
        {
            elina_double_interval_mul_cst_coeff_const_expr(&tmp1, &tmp2, prev_inf_coeff, prev_sup_coeff, input_ucsts[i]);
        ...
        {
            elina_double_interval_mul2(&tmp1, &tmp2, prev_inf_coeff, prev_sup_coeff, input_inf[i], input_sup[i]);
...
void ffn_handle_first_layer(elina_manager_t* man, elina_abstract0_t* abs, const double** weights, const double* bias, const size_t size, const size_t num_pixels, size_t* predecessors, const activation_type_t activation, const bool alloc)
{
    ...
    if((res->input_lweights != nullptr) && (res->input_uweights != nullptr) && (res->input_lcst != nullptr) && (res->input_ucst != nullptr))
    {
        ...
        cudaMalloc((void**) &linf_coeff, num_out_neurons_0_layer*mu*sizeof(float_type));
        cudaMalloc((void**) &lsup_coeff, num_out_neurons_0_layer*mu*sizeof(float_type));
        cudaMalloc((void**) &uinf_coeff, num_out_neurons_0_layer*mu*sizeof(float_type));
        cudaMalloc((void**) &usup_coeff, num_out_neurons_0_layer*mu*sizeof(float_type));

        cudaMalloc((void**) &linf_cst, num_out_neurons_0_layer*sizeof(float_type));
        cudaMalloc((void**) &lsup_cst, num_out_neurons_0_layer*sizeof(float_type));
        cudaMalloc((void**) &uinf_cst, num_out_neurons_0_layer*sizeof(float_type));
        cudaMalloc((void**) &usup_cst, num_out_neurons_0_layer*sizeof(float_type));
        ...
        compute_lb_from_expr<<<res->layers[0]->num_out_neurons, 1>>>(res->layers[0]->lb_array, linf_coeff, lsup_coeff, linf_cst, res->input_inf + num_in_neurons_0_layer, res->input_sup + num_in_neurons_0_layer, mu);
        compute_ub_from_expr<<<res->layers[0]->num_out_neurons, 1>>>(res->layers[0]->ub_array, uinf_coeff, usup_coeff, usup_cst, res->input_inf + num_in_neurons_0_layer, res->input_sup + num_in_neurons_0_layer, mu);
    ...
    {
        layer_compute_bounds_from_exprs<<<res->layers[0]->num_out_neurons, 1>>>(coeffs, csts, res->layers[0]->lb_array, res->layers[0]->ub_array, res->input_inf, res->input_sup, res->layers[0]->num_out_neurons, res->layers[0]->num_in_neurons);
    }
}
...
void lexpr_replace_relu_bounds(float_type* __restrict__ inf_coeff, float_type* __restrict__ sup_coeff, float_type* __restrict__ inf_cst, float_type* __restrict__ sup_cst, const float_type* __restrict__ lb_array, const float_type* __restrict__ ub_array, const size_t num_out_neurons_current_layer, const bool use_area_heuristic)
{
    ...
    for(size_t i = 0; i < num_out_neurons_current_layer; i++)
    {
        ...
        else
        {
            ...
            sup_coeff[a] = 0.0;
            float_type tmp1, tmp2;
            elina_double_interval_mul2(&tmp1, &tmp2, old_inf_coeff, old_sup_coeff, 0, ub);
...
// TODO: Assess if non-determinism introduced by atomics is problematic for analyzer.
__global__
void lexpr_replace_relu_bounds_conv_sparse(float_type* __restrict__ inf_coeff, float_type* __restrict__ sup_coeff, float_type* __restrict__ inf_cst, float_type* __restrict__ sup_cst, const float_type* __restrict__ lb_array, const float_type* __restrict__ ub_array, const size_t output_size_x, const size_t output_size_y, const size_t output_size_z, long int offset_x, long int offset_y, long int length_x, long int length_y, long int shift_x, long int shift_y, const bool use_area_heuristic)
{
    const size_t last_x = blockIdx.x;
    const size_t last_y = blockIdx.y;
    const size_t last_z = blockIdx.z;
...
__global__
void uexpr_replace_relu_bounds_conv_sparse(float_type* __restrict__ inf_coeff, float_type* __restrict__ sup_coeff, float_type* __restrict__ inf_cst, float_type* __restrict__ sup_cst, const float_type* __restrict__ lb_array, const float_type* __restrict__ ub_array, const size_t output_size_x, const size_t output_size_y, const size_t output_size_z, long int offset_x, long int offset_y, long int length_x, long int length_y, long int shift_x, long int shift_y, const bool use_area_heuristic)
{
    const size_t last_x = blockIdx.x;
    const size_t last_y = blockIdx.y;
    const size_t last_z = blockIdx.z;
            ...
            for(long int out_y = min_y; out_y < max_y; out_y++)
            {
                ...
                if(!((x_val < 0) || (x_val >= (long int)input_size_x)))
                {
                    if(!((y_val < 0) || (y_val >= (long int)input_size_y)))
                    {
                        ...
                        for(size_t out_z = 0; out_z < output_size_z; out_z++)
                        {
                            const size_t mat_out = out_x*length_y*output_size_z + out_y*output_size_z + out_z;
                            const size_t a = n*length_x*length_y*output_size_z + mat_out;
...
__global__
void lcoeffs_from_input_poly_sparse(const float_type* __restrict__ expr_inf_coeff, const float_type* __restrict__ expr_sup_coeff, float_type* __restrict__ res_inf_coeff, float_type* __restrict__ res_sup_coeff, const float_type* __restrict__ input_lcoeffs, const float_type* __restrict__ input_ucoeffs, const size_t output_size_x, const size_t output_size_y, const size_t output_size_z, const size_t num_in_neurons, const long int offset_x, const long int offset_y, const long int length_x, const long int length_y, const long int shift_x, const long int shift_y)
{
    const size_t last_x = blockIdx.x;
    const size_t last_y = blockIdx.y;
    const size_t last_z = blockIdx.z;
        ...
        for(long int out_x = min_x; out_x < max_x; out_x++)
        {
            for(long int out_y = min_y; out_y < max_y; out_y++)
            {
                for(size_t out_z = 0; out_z < output_size_z; out_z++)
                {
                    ...
                    const size_t c = ((out_x + min_out_x)*output_size_y*output_size_z + (out_y + min_out_y)*output_size_z + out_z)*num_in_neurons + i;
...
__global__
void csts_from_previous_layer(const float_type* __restrict__ expr_inf_coeff, const float_type* __restrict__ expr_sup_coeff, const float_type* __restrict__ expr_inf_cst, const float_type* __restrict__ expr_sup_cst, float_type* __restrict__ res_inf_cst, float_type* __restrict__ res_sup_cst, const float_type* __restrict__ aux_csts, const size_t num_out_neurons_current_layer)
{
    const size_t n = blockIdx.x;
    ...
    for(size_t i = 0; i < num_out_neurons_current_layer; i++)
    {
        ...
        if((prev_inf_coeff != 0) || (prev_sup_coeff != 0))
        {
            elina_double_interval_mul_cst_coeff_const_expr(&tmp1, &tmp2, prev_inf_coeff, prev_sup_coeff, aux_csts[i]);
...
__global__
void csts_from_previous_layer_conv(const float_type* __restrict__ expr_inf_coeff, const float_type* __restrict__ expr_sup_coeff, const float_type* __restrict__ expr_inf_cst, const float_type* __restrict__ expr_sup_cst, float_type* __restrict__ res_inf_cst, float_type* __restrict__ res_sup_cst, const float_type* __restrict__ aux_csts, const size_t current_layer_out_size_x, const size_t current_layer_out_size_y, const size_t current_layer_out_size_z)
{
    const size_t n = blockIdx.x;
...
__global__
void csts_from_previous_layer_conv_sparse(const float_type* __restrict__ expr_inf_coeff, const float_type* __restrict__ expr_sup_coeff, const float_type* __restrict__ expr_inf_cst, const float_type* __restrict__ expr_sup_cst, float_type* __restrict__ res_inf_cst, float_type* __restrict__ res_sup_cst, const float_type* __restrict__ aux_csts, const size_t output_size_x, const size_t output_size_y, const size_t output_size_z, long int offset_x, long int offset_y, long int length_x, long int length_y, long int shift_x, long int shift_y)
{
    const size_t last_x = blockIdx.x;
    const size_t last_y = blockIdx.y;
    const size_t last_z = blockIdx.z;
        ...
        for(long int out_y = min_y; out_y < max_y; out_y++)
        {
            for(size_t out_z = 0; out_z < output_size_z; out_z++)
            {
                ...
                if((prev_inf_coeff != 0) || (prev_sup_coeff != 0))
                {
                    ...
                    elina_double_interval_mul_cst_coeff_const_expr(&tmp1, &tmp2, prev_inf_coeff, prev_sup_coeff, aux_cst);
...
__global__
void lcsts_from_input_poly_sparse(const float_type* __restrict__ expr_inf_coeff, const float_type* __restrict__ expr_sup_coeff, const float_type* __restrict__ expr_inf_cst, const float_type* __restrict__ expr_sup_cst, float_type* __restrict__ res_inf_cst, float_type* __restrict__ res_sup_cst, const float_type* __restrict__ input_lcsts, const float_type* __restrict__ input_ucsts, const float_type* __restrict__ input_inf, const float_type* __restrict__ input_sup, const size_t output_size_x, const size_t output_size_y, const size_t output_size_z, long int offset_x, long int offset_y, long int length_x, long int length_y, long int shift_x, long int shift_y)
{
    const size_t last_x = blockIdx.x;
    const size_t last_y = blockIdx.y;
    const size_t last_z = blockIdx.z;
    ...
    for(long int out_x = min_x; out_x < max_x; out_x++)
    {
        for(long int out_y = min_y; out_y < max_y; out_y++)
        {
            for(size_t out_z = 0; out_z < output_size_z; out_z++)
            {
                const size_t j = out_x*length_y*output_size_z + out_y*output_size_z + out_z;
                const size_t i = (out_x + min_out_x)*output_size_y*output_size_z + (out_y + min_out_y)*output_size_z + out_z;

                const size_t mat_out = n*length_x*length_y*output_size_z + j;
    ...
    for(size_t x_shift = 0; x_shift < filter_size_x; x_shift++)
    {
        for(size_t y_shift = 0; y_shift < filter_size_y; y_shift++)
        {
            for(size_t inp_z = 0; inp_z < input_size_z; inp_z++)
            {
                ...
                const size_t mat_y = x_shift*filter_size_y*input_size_z + y_shift*input_size_z + inp_z;
                const size_t filter_index = (chunk_counter*gridDim.z + out_z)*filter_size_x*filter_size_y*input_size_z + x_shift*filter_size_y*input_size_z + y_shift*input_size_z + inp_z;
...
void update_state_using_predecessor_layer(fppoly_internal_t* pr, fppoly_t* fp, float_type** linf_coeff, float_type** lsup_coeff, float_type** linf_cst, float_type** lsup_cst, float_type** uinf_coeff, float_type** usup_coeff, float_type** uinf_cst, float_type** usup_cst, float_type** linf_coeff_tmp, float_type** lsup_coeff_tmp, float_type** linf_cst_tmp, float_type** lsup_cst_tmp, float_type** uinf_coeff_tmp, float_type** usup_coeff_tmp, float_type** uinf_cst_tmp, float_type** usup_cst_tmp, const size_t k, bool use_area_heuristic, const size_t num_out_neurons_last_layer)
{
    ...
    {
        lexpr_replace_relu_bounds<<<num_out_neurons_last_layer, 1>>>(*linf_coeff, *lsup_coeff, *linf_cst, *lsup_cst, aux_lb_array, aux_ub_array, num_out_neurons_current_layer, use_area_heuristic);
        uexpr_replace_relu_bounds<<<num_out_neurons_last_layer, 1>>>(*uinf_coeff, *usup_coeff, *uinf_cst, *usup_cst, aux_lb_array, aux_ub_array, num_out_neurons_current_layer, use_area_heuristic);
    }
    ...
    if(fp->layers[k]->type == CONV)
    {
        if(fp->layers[k]->input_size[0]*fp->layers[k]->input_size[1]*fp->layers[k]->input_size[2] >= 256)
        {
            ...
            {
                coeffs_from_previous_layer_conv_filter_serial<<<num_out_neurons_last_layer, 256>>>(*linf_coeff, *lsup_coeff, *linf_coeff_tmp, *lsup_coeff_tmp, aux_coeffs, fp->layers[k]->output_size[0], fp->layers[k]->output_size[1], fp->layers[k]->output_size[2], fp->layers[k]->input_size[0], fp->layers[k]->input_size[1], fp->layers[k]->input_size[2], fp->layers[k]->filter_size[0], fp->layers[k]->filter_size[1], fp->layers[k]->strides[0], fp->layers[k]->strides[1], fp->layers[k]->pad[0], fp->layers[k]->pad[1]);
                coeffs_from_previous_layer_conv_filter_serial<<<num_out_neurons_last_layer, 256>>>(*uinf_coeff, *usup_coeff, *uinf_coeff_tmp, *usup_coeff_tmp, aux_coeffs, fp->layers[k]->output_size[0], fp->layers[k]->output_size[1], fp->layers[k]->output_size[2], fp->layers[k]->input_size[0], fp->layers[k]->input_size[1], fp->layers[k]->input_size[2], fp->layers[k]->filter_size[0], fp->layers[k]->filter_size[1], fp->layers[k]->strides[0], fp->layers[k]->strides[1], fp->layers[k]->pad[0], fp->layers[k]->pad[1]);
            }
            else
            {
        ...
        csts_from_previous_layer_conv<<<num_out_neurons_last_layer, 1>>>(*linf_coeff, *lsup_coeff, *linf_cst, *lsup_cst, *linf_cst_tmp, *lsup_cst_tmp, aux_csts, fp->layers[k]->output_size[0], fp->layers[k]->output_size[1], fp->layers[k]->output_size[2]);
        csts_from_previous_layer_conv<<<num_out_neurons_last_layer, 1>>>(*uinf_coeff, *usup_coeff, *uinf_cst, *usup_cst, *uinf_cst_tmp, *usup_cst_tmp, aux_csts, fp->layers[k]->output_size[0], fp->layers[k]->output_size[1], fp->layers[k]->output_size[2]);
    ...
    else
    {
        ...
        coeffs_from_previous_layer<<<num_out_neurons_last_layer, num_threads>>>(*uinf_coeff, *usup_coeff, *uinf_coeff_tmp, *usup_coeff_tmp, aux_coeffs, num_out_neurons_current_layer, num_in_neurons_current_layer);

        csts_from_previous_layer<<<num_out_neurons_last_layer, 1>>>(*linf_coeff, *lsup_coeff, *linf_cst, *lsup_cst, *linf_cst_tmp, *lsup_cst_tmp, aux_csts, num_out_neurons_current_layer);
        csts_from_previous_layer<<<num_out_neurons_last_layer, 1>>>(*uinf_coeff, *usup_coeff, *uinf_cst, *usup_cst, *uinf_cst_tmp, *usup_cst_tmp, aux_csts, num_out_neurons_current_layer);
    }
...
void update_state_using_previous_layers(elina_manager_t* man, fppoly_t* fp, const size_t layerno, const bool use_area_heuristic)
{
    ...
    while(k >= 0)
    {
        if(fp->layers[k]->type == RESIDUAL)
        {
            if(fp->layers[k]->activation==RELU)
            {
                lexpr_replace_relu_bounds<<<num_out_neurons_last_layer, 1>>>(linf_coeff, lsup_coeff, linf_cst, lsup_cst, fp->layers[k]->lb_array, fp->layers[k]->ub_array, fp->layers[k]->num_out_neurons, use_area_heuristic);
                uexpr_replace_relu_bounds<<<num_out_neurons_last_layer, 1>>>(uinf_coeff, usup_coeff, uinf_cst, usup_cst, fp->layers[k]->lb_array, fp->layers[k]->ub_array, fp->layers[k]->num_out_neurons, use_area_heuristic);
            }
            ...
            cudaMalloc((void**) &copy_linf_coeff, num_out_neurons_last_layer*reslayer_size*sizeof(float_type));
            cudaMalloc((void**) &copy_lsup_coeff, num_out_neurons_last_layer*reslayer_size*sizeof(float_type));

            cudaMalloc((void**) &copy_linf_cst, num_out_neurons_last_layer*sizeof(float_type));
            cudaMalloc((void**) &copy_lsup_cst, num_out_neurons_last_layer*sizeof(float_type));
            ...
            cudaMalloc((void**) &copy_uinf_coeff, num_out_neurons_last_layer*reslayer_size*sizeof(float_type));
            cudaMalloc((void**) &copy_usup_coeff, num_out_neurons_last_layer*reslayer_size*sizeof(float_type));

            cudaMalloc((void**) &copy_uinf_cst, num_out_neurons_last_layer*sizeof(float_type));
            cudaMalloc((void**) &copy_usup_cst, num_out_neurons_last_layer*sizeof(float_type));
            ...
            cudaMalloc((void**) &copy_linf_cst_tmp, num_out_neurons_last_layer*sizeof(float_type));
            cudaMalloc((void**) &copy_lsup_cst_tmp, num_out_neurons_last_layer*sizeof(float_type));
            cudaMalloc((void**) &copy_uinf_cst_tmp, num_out_neurons_last_layer*sizeof(float_type));
            cudaMalloc((void**) &copy_usup_cst_tmp, num_out_neurons_last_layer*sizeof(float_type));
    ...
    if((fp->input_lweights != nullptr) && (fp->input_uweights != nullptr) && (fp->input_lcst != nullptr) && (fp->input_ucst != nullptr))
    {
        ...
        cudaMalloc((void**) &uinf_coeff_tmp, num_out_neurons_last_layer*mu*sizeof(float_type));
        cudaMalloc((void**) &usup_coeff_tmp, num_out_neurons_last_layer*mu*sizeof(float_type));
        ...
        compute_lb_from_expr<<<num_out_neurons_last_layer, 1>>>(lb_array, linf_coeff, lsup_coeff, linf_cst, fp->input_inf + num_in_neurons_0_layer, fp->input_sup + num_in_neurons_0_layer, mu);
        compute_ub_from_expr<<<num_out_neurons_last_layer, 1>>>(ub_array, uinf_coeff, usup_coeff, usup_cst, fp->input_inf + num_in_neurons_0_layer, fp->input_sup + num_in_neurons_0_layer, mu);
    }
    else
    {
        compute_lb_from_expr<<<num_out_neurons_last_layer, 1>>>(lb_array, linf_coeff, lsup_coeff, linf_cst, fp->input_inf, fp->input_sup, num_in_neurons_0_layer);
        compute_ub_from_expr<<<num_out_neurons_last_layer, 1>>>(ub_array, uinf_coeff, usup_coeff, usup_cst, fp->input_inf, fp->input_sup, num_in_neurons_0_layer);
    }
...
void update_state_using_predecessor_layer_conv_chunk(fppoly_internal_t* pr, fppoly_t* fp, float_type** linf_coeff, float_type** lsup_coeff, float_type** linf_cst, float_type** lsup_cst, float_type** uinf_coeff, float_type** usup_coeff, float_type** uinf_cst, float_type** usup_cst, float_type** linf_coeff_tmp, float_type** lsup_coeff_tmp, float_type** linf_cst_tmp, float_type** lsup_cst_tmp, float_type** uinf_coeff_tmp, float_type** usup_coeff_tmp, float_type** uinf_cst_tmp, float_type** usup_cst_tmp, const size_t layerno, const size_t k, bool use_area_heuristic, const size_t x_y_size_last_layer, const size_t num_filters_last_layer, const size_t num_chunks, long int& offset_x, long int& offset_y, long int& length_x, long int& length_y, long int& shift_x, long int& shift_y)
{
    ...
    cudaMalloc((void**) &(*linf_coeff_tmp), x_y_size_last_layer*num_filters_last_layer*missing_length*sizeof(float_type));
    cudaMalloc((void**) &(*lsup_coeff_tmp), x_y_size_last_layer*num_filters_last_layer*missing_length*sizeof(float_type));
    cudaMalloc((void**) &(*uinf_coeff_tmp), x_y_size_last_layer*num_filters_last_layer*missing_length*sizeof(float_type));
    cudaMalloc((void**) &(*usup_coeff_tmp), x_y_size_last_layer*num_filters_last_layer*missing_length*sizeof(float_type));
...
void update_state_using_previous_layers_conv_chunk(elina_manager_t* man, fppoly_t* fp, const size_t layerno, const size_t num_chunks, const size_t chunk_counter, const bool use_area_heuristic)
{
    ...
    while(k >= 0)
    {
        if(fp->layers[k]->type == RESIDUAL)
        {
            ...
            if(backstep_counter >= maximum_backstep)
            {
                ...
                compute_lb_from_expr_conv_sparse<<<dim3(fp->layers[layerno]->output_size[0], fp->layers[layerno]->output_size[1], fp->layers[layerno]->output_size[2]/num_chunks), 1>>>(lb_array, linf_coeff, lsup_coeff, linf_cst, fp->layers[k]->lb_array, fp->layers[k]->ub_array, num_chunks, chunk_counter, fp->layers[k]->output_size[0], fp->layers[k]->output_size[1], fp->layers[k]->output_size[2], offset_x, offset_y, length_x, length_y, shift_x, shift_y);
                compute_ub_from_expr_conv_sparse<<<dim3(fp->layers[layerno]->output_size[0], fp->layers[layerno]->output_size[1], fp->layers[layerno]->output_size[2]/num_chunks), 1>>>(ub_array, uinf_coeff, usup_coeff, usup_cst, fp->layers[k]->lb_array, fp->layers[k]->ub_array, num_chunks, chunk_counter, fp->layers[k]->output_size[0], fp->layers[k]->output_size[1], fp->layers[k]->output_size[2], offset_x, offset_y, length_x, length_y, shift_x, shift_y);
    ...
    if(backstep_counter < maximum_backstep)
    {
        if((fp->input_lweights != nullptr) && (fp->input_uweights != nullptr) && (fp->input_lcst != nullptr) && (fp->input_ucst != nullptr))
        {
            ...
            cudaMalloc((void**) &linf_coeff_tmp, x_y_size_last_layer*num_filters_last_layer*mu*sizeof(float_type));
            cudaMalloc((void**) &lsup_coeff_tmp, x_y_size_last_layer*num_filters_last_layer*mu*sizeof(float_type));
            cudaMalloc((void**) &uinf_coeff_tmp, x_y_size_last_layer*num_filters_last_layer*mu*sizeof(float_type));
            cudaMalloc((void**) &usup_coeff_tmp, x_y_size_last_layer*num_filters_last_layer*mu*sizeof(float_type));
...
void ffn_handle_last_layer(elina_manager_t* man, elina_abstract0_t* element, const double** weights, const double* bias, const size_t num_out_neurons, const size_t num_in_neurons, size_t* predecessors, const bool has_activation, const activation_type_t activation, const bool alloc, const bool use_area_heuristic)
{
    ...
    float_type* lb_array_host = (float_type*) malloc(fp->layers[fp->numlayers - 1]->num_out_neurons*sizeof(float_type));
    float_type* ub_array_host = (float_type*) malloc(fp->layers[fp->numlayers - 1]->num_out_neurons*sizeof(float_type));

    cudaMemcpy(lb_array_host, lb_array, fp->layers[fp->numlayers - 1]->num_out_neurons*sizeof(float_type), cudaMemcpyDeviceToHost);
    cudaMemcpy(ub_array_host, ub_array, fp->layers[fp->numlayers - 1]->num_out_neurons*sizeof(float_type), cudaMemcpyDeviceToHost);
...
void get_lb_using_previous_layers(elina_manager_t* man, fppoly_t* const fp, const bool use_area_heuristic)
{
    ...
    cudaMalloc((void**) &linf_coeff, num_out_neurons_last_layer*10*sizeof(float_type));
    cudaMalloc((void**) &lsup_coeff, num_out_neurons_last_layer*10*sizeof(float_type));
    cudaMalloc((void**) &linf_cst, num_out_neurons_last_layer*10*sizeof(float_type));
    cudaMalloc((void**) &lsup_cst, num_out_neurons_last_layer*10*sizeof(float_type));
    ...
    cudaMalloc((void**) &linf_coeff_tmp, num_out_neurons_last_layer*sizeof(float_type));
    cudaMalloc((void**) &lsup_coeff_tmp, num_out_neurons_last_layer*sizeof(float_type));
    cudaMalloc((void**) &linf_cst_tmp, num_out_neurons_last_layer*sizeof(float_type));
    cudaMalloc((void**) &lsup_cst_tmp, num_out_neurons_last_layer*sizeof(float_type));
    ...
    cudaMemcpy(&lb, lb_dev, num_out_neurons_last_layer*sizeof(float_type), cudaMemcpyDeviceToHost);
...
void conv_add_layer(fppoly_t* const fp, const size_t num_out_neurons, const size_t num_in_neurons, const size_t num_nonzero_weights, const size_t num_biases, const layertype_t type, const activation_type_t activation)
{
    ...
    cudaMalloc((void**) &layer->filter_weights, num_nonzero_weights*sizeof(float_type));
...
void layer_create_sparse_exprs(fppoly_t* const fp, const double* filter_weights, const double* filter_bias, const size_t* input_size, const size_t* filter_size, const size_t num_filters, const size_t* strides, const bool is_valid_padding, const bool has_bias, const activation_type_t activation)
{
    ...
    cudaMemcpy(current_layer->filter_weights, filter_weights_tmp, size*sizeof(float_type), cudaMemcpyHostToDevice);

    if(has_bias)
    {
        cudaMemcpy(current_layer->filter_bias, filter_bias_tmp, output_size[2]*sizeof(float_type), cudaMemcpyHostToDevice);
    }

    cudaMemcpy(current_layer->input_size, input_size, 3*sizeof(size_t), cudaMemcpyHostToHost);
    cudaMemcpy(current_layer->output_size, output_size, 3*sizeof(size_t), cudaMemcpyHostToHost);
    cudaMemcpy(current_layer->filter_size, filter_size, 2*sizeof(size_t), cudaMemcpyHostToHost);
    cudaMemcpy(current_layer->strides, strides, 2*sizeof(size_t), cudaMemcpyHostToHost);
    cudaMemcpy(current_layer->pad, pad, 2*sizeof(long int), cudaMemcpyHostToHost);
...
void conv_handle_first_layer(elina_manager_t* man, elina_abstract0_t* element, const double* filter_weights, const double* filter_bias, const size_t* input_size, const size_t* filter_size, const size_t num_filters, const size_t* strides, const bool is_valid_padding, const bool has_bias, size_t* predecessors)
{
    ...
    if((fp->input_lweights != nullptr) && (fp->input_uweights != nullptr) && (fp->input_lcst != nullptr) && (fp->input_ucst != nullptr))
    {
        ...
        cudaMalloc((void**) &coeffs, num_out_neurons_0_layer*length_x*length_y*fp->layers[0]->input_size[2]*sizeof(float_type));
        ...
        compute_lb_from_expr<<<num_out_neurons_0_layer, 1>>>(fp->layers[0]->lb_array, linf_coeff, lsup_coeff, linf_cst, fp->input_inf + num_in_neurons_0_layer, fp->input_sup + num_in_neurons_0_layer, mu);
        compute_ub_from_expr<<<num_out_neurons_0_layer, 1>>>(fp->layers[0]->ub_array, uinf_coeff, usup_coeff, usup_cst, fp->input_inf + num_in_neurons_0_layer, fp->input_sup + num_in_neurons_0_layer, mu);
...
void res_add_layer(fppoly_t* const fp, const size_t num_neurons, const layertype_t type, const activation_type_t activation)
{
    ...
    cudaMalloc((void**) &layer->lb_array, layer->num_out_neurons*sizeof(float_type));
    cudaMalloc((void**) &layer->ub_array, layer->num_out_neurons*sizeof(float_type));
...
elina_interval_t** box_for_layer(elina_manager_t* man, elina_abstract0_t* abs, const size_t layerno)
{
    ...
    cudaMemcpy(lb_array_host, lb_array, num_neurons*sizeof(float_type), cudaMemcpyDeviceToHost);
    cudaMemcpy(ub_array_host, ub_array, num_neurons*sizeof(float_type), cudaMemcpyDeviceToHost);
