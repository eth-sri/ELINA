...
__global__
void compute_lb_from_expr_conv_sparse(float_type* __restrict__ lb_array, const float_type* __restrict__ inf_coeff, const float_type* __restrict__ sup_coeff, const float_type* __restrict__ inf_cst, const float_type* __restrict__ input_inf, const float_type* __restrict__ input_sup, const size_t num_out_neurons_last_layer, const size_t output_size_x, const size_t output_size_y, const size_t output_size_z, long int offset_x, long int offset_y, long int length_x, long int length_y, long int shift_x, long int shift_y)
{
...
// TODO: Assess if non-determinism introduced by atomics is problematic for analyzer.
__global__
void lexpr_replace_relu_bounds_conv_sparse(float_type* __restrict__ inf_coeff, float_type* __restrict__ sup_coeff, float_type* __restrict__ inf_cst, float_type* __restrict__ sup_cst, const float_type* __restrict__ lb_array, const float_type* __restrict__ ub_array, const size_t num_out_neurons_last_layer, const size_t output_size_x, const size_t output_size_y, const size_t output_size_z, long int offset_x, long int offset_y, long int length_x, long int length_y, long int shift_x, long int shift_y)
{
    const size_t last_x = blockIdx.x;
    const size_t last_y = blockIdx.y;
    const size_t last_z = blockIdx.z;
...
__global__
void csts_from_previous_layer_conv(const float_type* __restrict__ expr_inf_coeff, const float_type* __restrict__ expr_sup_coeff, const float_type* __restrict__ expr_inf_cst, const float_type* __restrict__ expr_sup_cst, float_type* __restrict__ res_inf_cst, float_type* __restrict__ res_sup_cst, const float_type* __restrict__ aux_csts, const size_t num_out_neurons_last_layer, const size_t current_layer_out_size_x, const size_t current_layer_out_size_y, const size_t current_layer_out_size_z)
{
    const size_t n = blockIdx.x;
...
__global__
void csts_from_previous_layer_conv_sparse(const float_type* __restrict__ expr_inf_coeff, const float_type* __restrict__ expr_sup_coeff, const float_type* __restrict__ expr_inf_cst, const float_type* __restrict__ expr_sup_cst, float_type* __restrict__ res_inf_cst, float_type* __restrict__ res_sup_cst, const float_type* __restrict__ aux_csts, const size_t num_out_neurons_last_layer, const size_t output_size_x, const size_t output_size_y, const size_t output_size_z, long int offset_x, long int offset_y, long int length_x, long int length_y, long int shift_x, long int shift_y)
{
    const size_t last_x = blockIdx.x;
    const size_t last_y = blockIdx.y;
    const size_t last_z = blockIdx.z;
...
void device_layer_create_sparse_exprs(float_type* dense_coeff, float_type* bias, const float_type* filter_weights,
                                      const float_type* filter_bias, const size_t input_size_x, const size_t input_size_y, const size_t input_size_z, const size_t output_size_x, const size_t output_size_y, const size_t output_size_z, const size_t filter_size_x, const size_t filter_size_y,
                                      const size_t stride_x, const size_t stride_y, const long int pad_x, const long int pad_y, const size_t num_pixels)
{
    ...
    const size_t mat_x = out_x*output_size_y*output_size_z + out_y*output_size_z + out_z;
...
{
    device_layer_create_sparse_exprs<<<dim3(layer->output_size[0], layer->output_size[1], layer->output_size[2]), 1>>>(coeffs, csts, layer->filter_weights, layer->filter_bias, layer->input_size[0], layer->input_size[1], layer->input_size[2], layer->output_size[0], layer->output_size[1], layer->output_size[2], layer->filter_size[0], layer->filter_size[1], layer->strides[0], layer->strides[1], layer->pad[0], layer->pad[1], layer->num_in_neurons);
}
...
void update_state_using_previous_layers(elina_manager_t* man, fppoly_t* fp, const size_t layerno)
{
    ...
    for(int k = layerno - 1; k >= 0; k--)
    {
        ...
        else
        {
            ...
            csts_from_previous_layer<<<num_out_neurons_last_layer, 1>>>(linf_coeff, lsup_coeff, linf_cst, lsup_cst, linf_cst_tmp, lsup_cst_tmp, aux_csts, num_out_neurons_last_layer, num_out_neurons_current_layer);
            csts_from_previous_layer<<<num_out_neurons_last_layer, 1>>>(uinf_coeff, usup_coeff, uinf_cst, usup_cst, uinf_cst_tmp, usup_cst_tmp, aux_csts, num_out_neurons_last_layer, num_out_neurons_current_layer);
        }
...
void get_lb_using_previous_layers(elina_manager_t* man, const fppoly_t* const fp)
{
    ...
    for(int k = numlayers - 1; k >= 0; k--)
    {
        ...
        if(fp->layers[k]->type == CONV)
        {
            cudaMalloc((void**) &aux_coeffs, fp->layers[k]->num_out_neurons*fp->layers[k]->num_in_neurons*sizeof(float_type));
            cudaMalloc((void**) &aux_csts, fp->layers[k]->num_out_neurons*sizeof(float_type));
