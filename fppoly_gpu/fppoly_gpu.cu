...
void layer_create_exprs(neuron_t** neurons, const double** weights, const double* bias, const size_t num_out_neurons, const size_t num_in_neurons)
{
...
__device__ __host__
void lexpr_replace_relu_bounds(expr_t** expr_ptr, neuron_t** neurons)
{
...
__device__ __host__
void uexpr_replace_relu_bounds(expr_t** expr_ptr, neuron_t** neurons)
{
    expr_t* expr = *expr_ptr;
    const size_t num_neurons = expr->size;
...
__device__ __host__
void lexpr_replace_maxpool_bounds(expr_t** expr_ptr, neuron_t** neurons)
{
    //printf("begin\n");
    //fflush(stdout);
    expr_t* expr = *expr_ptr;
...
__device__ __host__
void uexpr_replace_maxpool_bounds(expr_t** expr_ptr, neuron_t** neurons)
{
    expr_t* expr = *expr_ptr;
    const size_t num_neurons = expr->size;
    expr_t* res;
...
__device__ __host__
void expr_from_previous_layer(expr_t** expr_ptr, neuron_t** neurons)
{
    expr_t* expr = *expr_ptr;
...
void ffn_handle_intermediate_layer(elina_manager_t* man, elina_abstract0_t* element, const double** weights, const double* bias, const size_t num_out_neurons, const size_t num_in_neurons, const activation_type_t activation)
{
    ...
    layer_create_exprs(out_neurons, weights, bias, num_out_neurons, num_in_neurons);
...
__device__ __host__
double apply_relu_lexpr(expr_t** lexpr_p, const neuron_t* const neuron)
{
...
__device__ __host__
double apply_relu_uexpr(expr_t** uexpr_p, neuron_t* neuron)
{
...
double get_lb_using_previous_layers(elina_manager_t* man, const fppoly_t* const fp, expr_t* expr)
{
    const size_t numlayers = fp->numlayers;
    expr_t* lexpr = expr;
    expr_t** lexpr_ptr = &lexpr;
...
void conv_handle_first_layer(elina_manager_t* man, elina_abstract0_t* abs, const double* filter_weights, const double* filter_bias,
    ...
    for(out_x = 0; out_x < output_size[0]; out_x++)
    {
        for(out_y = 0; out_y < output_size[1]; out_y++)
        {
            for(out_z = 0; out_z < output_size[2]; out_z++)
            {
                ...
                neurons[mat_x]->lb = compute_lb_from_expr(neurons[mat_x]->expr, res->input_inf, res->input_sup, res->input_lexpr, res->input_uexpr);
                neurons[mat_x]->ub = compute_ub_from_expr(neurons[mat_x]->expr, res->input_inf, res->input_sup, res->input_lexpr, res->input_uexpr);
