...
__global__ void compute_lb_from_expr_input_poly_sparse(
    ...
    const float_type *__restrict__ input_inf,
    const float_type *__restrict__ input_sup, const size_t num_chunks,
    const size_t chunk_counter, const size_t mu) {
  const size_t last_x = blockIdx.x;
  const size_t last_y = blockIdx.y;
  const size_t last_z = blockIdx.z;
...
__global__ void copy_coeffs_and_csts(
    ...
    const float_type *__restrict__ source_csts, const size_t coeff_size) {
  const size_t i = blockIdx.x;
...
__global__ void add_coeffs_and_csts_sparse(
    ...
    const float_type *__restrict__ source_inf_cst,
    const float_type *__restrict__ source_sup_cst,
    const size_t output_size_target_x, const size_t output_size_target_y,
    const size_t output_size_source_x, const size_t output_size_source_y,
    const size_t output_size_z, const size_t relative_offset_x,
    const size_t relative_offset_y) {
  ...
  const size_t num_coeffs_target =
      output_size_target_x * output_size_target_y * output_size_z;
  const size_t num_coeffs_source =
  ...
  for (size_t out_x = 0; out_x < output_size_source_x; out_x++) {
    for (size_t out_y = 0; out_y < output_size_source_y; out_y++) {
      for (size_t out_z = 0; out_z < output_size_z; out_z++) {
        ...
        const size_t source_idx = out_x * output_size_source_y * output_size_z +
...
__global__ void lexpr_replace_relu_bounds_conv_sparse(
    ...
    const size_t output_size_y, const size_t output_size_z, long int offset_x,
    long int offset_y, long int length_x, long int length_y, long int shift_x,
    long int shift_y, const bool use_area_heuristic) {
  const size_t last_x = blockIdx.x;
  const size_t last_y = blockIdx.y;
  const size_t last_z = blockIdx.z;
          ...
          if (use_area_heuristic) {
            ...
            if ((area1 < area2) && (area1 < area3)) {
              elina_double_interval_mul_expr_coeff(
                  &inf_coeff[a], &sup_coeff[a], lambda_inf, lambda_sup,
                  old_inf_coeff, old_sup_coeff);
            } else if ((area2 < area1) && (area2 < area3)) {
              inf_coeff[a] = 0.0;
              sup_coeff[a] = 0.0;
...
__global__ void coeffs_from_previous_layer_conv_sparse(
    ...
    const size_t output_size_y, const size_t output_size_z,
    const size_t input_size_x, const size_t input_size_y,
    const size_t input_size_z, const long int offset_x, const long int offset_y,
    const long int length_x, const long int length_y, const long int shift_x,
    const long int shift_y, const long int new_length_x,
    const long int new_length_y, const size_t filter_size_x,
    const size_t filter_size_y, const size_t stride_x, const size_t stride_y,
    const size_t pad_x, const size_t pad_y) {
  const size_t last_x = blockIdx.x;
  const size_t last_y = blockIdx.y;
...
__global__ void lcoeffs_from_input_poly_sparse(
    ...
    float_type *__restrict__ res_sup_coeff,
    const float_type *__restrict__ input_lcoeffs,
    const float_type *__restrict__ input_ucoeffs, const size_t output_size_x,
    const size_t output_size_y, const size_t output_size_z,
    const size_t num_in_neurons, const long int offset_x,
    const long int offset_y, const long int length_x, const long int length_y,
    const long int shift_x, const long int shift_y) {
  const size_t last_x = blockIdx.x;
  const size_t last_y = blockIdx.y;
  const size_t last_z = blockIdx.z;
...
__global__ void
csts_from_previous_layer_conv(const float_type *__restrict__ expr_linf_coeff,
                              const float_type *__restrict__ expr_lsup_coeff,
                              const float_type *__restrict__ expr_uinf_coeff,
                              const float_type *__restrict__ expr_usup_coeff,
                              const float_type *__restrict__ expr_linf_cst,
  ...
  for (long int out_x = min_x; out_x < max_x; out_x++) {
    for (long int out_y = min_y; out_y < max_y; out_y++) {
      for (size_t out_z = 0; out_z < output_size_z; out_z++) {
        const size_t j =
            out_x * length_y * output_size_z + out_y * output_size_z + out_z;
        const size_t i = (out_x + min_out_x) * output_size_y * output_size_z +
                         (out_y + min_out_y) * output_size_z + out_z;
...
void update_state_using_predecessor_layer(
  ...
  if (fp->layers[k]->activation == RELU) {
    ...
    // lexpr_replace_relu_bounds<<<num_out_neurons_last_layer,
    // num_threads>>>(*linf_coeff, *lsup_coeff, *linf_cst, *lsup_cst,
    // aux_lb_array, aux_ub_array, num_out_neurons_current_layer,
    // use_area_heuristic);
    // uexpr_replace_relu_bounds<<<num_out_neurons_last_layer,
    // num_threads>>>(*uinf_coeff, *usup_coeff, *uinf_cst, *usup_cst,
    // aux_lb_array, aux_ub_array, num_out_neurons_current_layer,
    // use_area_heuristic);
  }
      ...
      if (fp->layers[k]->input_size[1] * fp->layers[k]->input_size[2] > 256) {
        if (fp->layers[k]->input_size[2] > 256) {
          coeffs_from_previous_layer_conv_filter_serial<<<
              num_out_neurons_last_layer, 256>>>(
  ...
  while (k >= 0) {
    if (fp->layers[k]->type == RESIDUAL) {
      if (fp->layers[k]->activation == RELU) {
        ...
        // fp->layers[k]->num_out_neurons, use_area_heuristic);
        // uexpr_replace_relu_bounds<<<num_out_neurons_last_layer,
        // num_threads>>>(uinf_coeff, usup_coeff, uinf_cst, usup_cst,
        // fp->layers[k]->lb_array, fp->layers[k]->ub_array,
        // fp->layers[k]->num_out_neurons, use_area_heuristic);
...
void update_state_using_predecessor_layer_sparse(
  ...
  if (fp->layers[k]->activation == RELU) {
    ...
    // lexpr_replace_relu_bounds_conv_sparse<<<dim3(fp->layers[layerno]->output_size[0],
    // fp->layers[layerno]->output_size[1],
    // fp->layers[layerno]->output_size[2]/num_chunks),
    // fp->layers[k]->output_size[2]>>>(*linf_coeff, *lsup_coeff, *linf_cst,
    // *lsup_cst, aux_lb_array, aux_ub_array, fp->layers[k]->output_size[0],
    ...
    // uexpr_replace_relu_bounds_conv_sparse<<<dim3(fp->layers[layerno]->output_size[0],
    // fp->layers[layerno]->output_size[1],
    // fp->layers[layerno]->output_size[2]/num_chunks),
    // fp->layers[k]->output_size[2]>>>(*uinf_coeff, *usup_coeff, *uinf_cst,
    // *usup_cst, aux_lb_array, aux_ub_array, fp->layers[k]->output_size[0],
    // fp->layers[k]->output_size[1], fp->layers[k]->output_size[2], offset_x,
    // offset_y, length_x, length_y, shift_x, shift_y, use_area_heuristic);
  }
      ...
      if (fp->layers[k]->input_size[2] > 256) {
        coeffs_from_previous_layer_conv_sparse_filter_serial<<<
            dim3(fp->layers[layerno]->output_size[0],
                 fp->layers[layerno]->output_size[1],
                 fp->layers[layerno]->output_size[2] / num_chunks),
            256>>>(*linf_coeff, *lsup_coeff, *uinf_coeff, *usup_coeff,
                   *linf_coeff_tmp, *lsup_coeff_tmp, *uinf_coeff_tmp,
                   *usup_coeff_tmp, aux_coeffs, fp->layers[k]->output_size[0],
                   fp->layers[k]->output_size[1], fp->layers[k]->output_size[2],
                   fp->layers[k]->input_size[0], fp->layers[k]->input_size[1],
...
void update_state_using_previous_layers_sparse(elina_manager_t *man,
                                               fppoly_t *fp,
                                               const size_t layerno,
                                               const size_t num_chunks,
                                               const size_t chunk_counter,
  ...
  while (k >= 0) {
    if (fp->layers[k]->type == RESIDUAL) {
      if (fp->layers[k]->activation == RELU) {
        ...
        // fp->layers[layerno]->output_size[2]/num_chunks),
        // fp->layers[k]->output_size[2]>>>(linf_coeff, lsup_coeff, linf_cst,
        // lsup_cst, fp->layers[k]->lb_array, fp->layers[k]->ub_array,
        // fp->layers[k]->output_size[0], fp->layers[k]->output_size[1],
        // fp->layers[k]->output_size[2], offset_x, offset_y, length_x, length_y,
        // shift_x, shift_y, use_area_heuristic);
        // uexpr_replace_relu_bounds_conv_sparse<<<dim3(fp->layers[layerno]->output_size[0],
        ...
        // fp->layers[layerno]->output_size[2]/num_chunks),
        // fp->layers[k]->output_size[2]>>>(uinf_coeff, usup_coeff, uinf_cst,
        // usup_cst, fp->layers[k]->lb_array, fp->layers[k]->ub_array,
        // fp->layers[k]->output_size[0], fp->layers[k]->output_size[1],
        // fp->layers[k]->output_size[2], offset_x, offset_y, length_x, length_y,
        // shift_x, shift_y, use_area_heuristic);
      }
...
void ffn_handle_intermediate_layer(
    elina_manager_t *man, elina_abstract0_t *element, const double **weights,
...
void ffn_handle_last_relu_layer(elina_manager_t *man,
                                ...
                                const size_t num_out_neurons,
                                const size_t num_in_neurons,
                                size_t *predecessors, const bool has_relu,
                                const bool use_area_heuristic) {
