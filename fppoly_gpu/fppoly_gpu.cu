...
bool is_greater(elina_manager_t *man, elina_abstract0_t *element, elina_dim_t y,
  ...
  (double*)malloc(sizeA*sizeof(double)); sub->dim = nullptr; sub->size = sizeA;
                          sub->type = DENSE;
                          if(exprB->type==DENSE){
                                          for(i=0; i < sizeA; i++){
                                                  sub->inf_coeff[i] =
  ...
  (double*)malloc(sizeB*sizeof(double)); sub->dim = nullptr; sub->size = sizeB;
                                  sub->type = DENSE;
                                  i = 0;
                                  for(k=0; k < sizeB; k++){
                                          if(i < sizeA && exprA->dim[i]==k){
...
create_sparse_expr(coeff,0,dim,1);
                        ...
                        //fflush(stdout);
                        //expr_t * expr = (expr_t *)malloc(sizeof(expr_t));
                        //expr->inf_coeff= expr->sup_coeff = expr->dim =
nullptr;
                        //expr->size = 0;
                        //expr->inf_cst = -max_l;
