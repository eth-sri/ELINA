...
__global__ void device_layer_create_dense_expr(
    ...
    const float_type *__restrict__ weights, const float_type *__restrict__ bias,
...
__global__ void lcoeffs_from_previous_layer_pool(
    ...
    const float_type *__restrict__ aux_lcoeffs,
    const float_type *__restrict__ aux_ucoeffs, const int output_size_x,
    const int output_size_y, const int output_size_z, const int input_size_x,
    const int input_size_y, const int filter_size_x, const int filter_size_y,
          ...
          for (size_t y_shift = 0; y_shift < filter_size_y; y_shift++) {
            ...
            if (!((y_val < 0) || (y_val >= input_size_y))) {
              if (!((x_val < 0) || (x_val >= input_size_x))) {
                const int mat_in = x_val * input_size_y * output_size_z +
                                   y_val * output_size_z + out_z;
                const int b =
                    n * input_size_x * input_size_y * output_size_z + mat_in;
                ...
                const int c =
                    out_x * output_size_y * filter_size_x * filter_size_y *
                        output_size_z +
                    out_y * filter_size_x * filter_size_y * output_size_z +
                    (x_shift * filter_size_y + y_shift) * output_size_z + out_z;
                const float_type prev_inf_coeff = expr_inf_coeff[a];
                const float_type prev_sup_coeff = expr_sup_coeff[a];

                if (prev_inf_coeff > 0) {
                  affine_trans_coeff(res_inf_coeff[b], res_sup_coeff[b], tmp1,
                                     tmp2, maxRes, maxMul, prev_inf_coeff,
                                     prev_sup_coeff, aux_lcoeffs[c]);
                }

                if (prev_sup_coeff < 0) {
                  affine_trans_coeff(res_inf_coeff[b], res_sup_coeff[b], tmp1,
                                     tmp2, maxRes, maxMul, prev_inf_coeff,
                                     prev_sup_coeff, aux_ucoeffs[c]);
                }
              }
            }
          }
...
__global__ void ucoeffs_from_previous_layer_pool(
    const float_type *__restrict__ expr_inf_coeff,
    const float_type *__restrict__ expr_sup_coeff,
    float_type *__restrict__ res_inf_coeff,
    float_type *__restrict__ res_sup_coeff,
...
void ffn_handle_first_layer(elina_manager_t *man, elina_abstract0_t *abs,
  ...
  if (OP == MUL || OP == SUB1 || OP == SUB2) {
    ...
    if (OP == MUL) {
      float_type *dense_cst =
          (float_type *)calloc(num_pixels, sizeof(float_type));

      for (size_t j = 0; j < num_pixels; j++) {
        dense_weights[j][j] = cst[j];
      }

      layer_create_dense_exprs(coeffs, csts, dense_weights, dense_cst,
      ...
      layer_create_dense_exprs(coeffs, csts, dense_weights, cst, num_pixels,
                               num_pixels);
    } else if (OP == SUB2) {
      float_type *dense_cst =
          (float_type *)calloc(num_pixels, sizeof(float_type));
...
void ffn_handle_first_relu_layer(elina_manager_t *man, elina_abstract0_t *abs,
  ...
  ffn_handle_first_layer(man, abs, weights, bias, size, num_pixels,
                         predecessors, RELU, true, MATMULT);
}
...
void ffn_handle_first_sub_layer(elina_manager_t *man, elina_abstract0_t *abs,
  ...
  if (is_minuend == true) {
    ffn_handle_first_layer(man, abs, NULL, cst, size, size, predecessors, NONE,
                           true, SUB1);
  } else {
    ffn_handle_first_layer(man, abs, NULL, cst, size, size, predecessors, NONE,
                           true, SUB2);
...
void ffn_handle_first_mul_layer(elina_manager_t *man, elina_abstract0_t *abs,
                                const float_type *bias, const size_t size,
                                size_t *predecessors) {
  ffn_handle_first_layer(man, abs, NULL, bias, size, size, predecessors, NONE,
                         true, MUL);
}
...
__global__ void lcoeffs_from_previous_layer_pool_sparse(
    const float_type *__restrict__ expr_inf_coeff,
    const float_type *__restrict__ expr_sup_coeff,
    float_type *__restrict__ res_inf_coeff,
    float_type *__restrict__ res_sup_coeff,
    ...
    const float_type *__restrict__ aux_ucoeffs, const int output_size_x,
    const int output_size_y, const int output_size_z, const int input_size_x,
    const int input_size_y, const int offset_x, const int offset_y,
    const int length_x, const int length_y, const int shift_x,
    const int shift_y, const int new_length_x, const int new_length_y,
        ...
        for (size_t x_shift = 0; x_shift < filter_size_x; x_shift++) {
          for (size_t y_shift = 0; y_shift < filter_size_y; y_shift++) {
            ...
            if (!((x_val < 0) || (x_val >= input_size_x))) {
              if (!((y_val < 0) || (y_val >= input_size_y))) {
                const int mat_in =
                    (out_x * stride_x + x_shift) * new_length_y *
                        output_size_z +
                    (out_y * stride_y + y_shift) * output_size_z + out_z;
                const int b =
                    n * new_length_x * new_length_y * output_size_z + mat_in;
                ...
                const int c =
                    (out_x + min_out_x) * output_size_y * filter_size_x *
                        filter_size_y * output_size_z +
                    (out_y + min_out_y) * filter_size_x * filter_size_y *
                        output_size_z +
                    (x_shift * filter_size_y + y_shift) * output_size_z + out_z;
...
__global__ void ucoeffs_from_previous_layer_pool_sparse(
    const float_type *__restrict__ expr_inf_coeff,
    const float_type *__restrict__ expr_sup_coeff,
    float_type *__restrict__ res_inf_coeff,
    float_type *__restrict__ res_sup_coeff,
...
void update_state_using_predecessor_layer(
    fppoly_t *fp, float_type **linf_coeff, float_type **lsup_coeff,
    float_type **linf_cst, float_type **lsup_cst, float_type **uinf_coeff,
    float_type **usup_coeff, float_type **uinf_cst, float_type **usup_cst,
    float_type **linf_coeff_tmp, float_type **lsup_coeff_tmp,
    float_type **linf_cst_tmp, float_type **lsup_cst_tmp,
    float_type **uinf_coeff_tmp, float_type **usup_coeff_tmp,
    float_type **uinf_cst_tmp, float_type **usup_cst_tmp, const size_t k,
  ...
  if (fp->layers[k]->activation == RELU) {
    expr_replace_relu_bounds<<<num_out_neurons_last_layer, num_threads>>>(
        ...
        *lsup_cst, *uinf_cst, *usup_cst, fp->layers[k]->lb_array,
        fp->layers[k]->ub_array, num_out_neurons_current_layer,
        use_area_heuristic);
  }
  ...
  *usup_coeff_tmp = malloc_device<float_type>(num_out_neurons_last_layer *
  ...
  if (fp->layers[k]->type == CONV) {
      ...
      if (fp->layers[k]->input_size[1] * fp->layers[k]->input_size[2] >
        ...
        if (fp->layers[k]->input_size[2] > num_threads) {
          coeffs_from_previous_layer_conv_filter_serial<<<
              ...
              *usup_coeff_tmp, fp->layers[k]->filter_weights,
        ...
        coeffs_from_previous_layer_conv_x_filter_serial<<<
            ...
            *lsup_coeff_tmp, *uinf_coeff_tmp, *usup_coeff_tmp,
    ...
    csts_from_previous_layer_conv<<<num_out_neurons_last_layer,
        ...
        *uinf_cst_tmp, *usup_cst_tmp, fp->layers[k]->filter_bias,
        fp->layers[k]->output_size[0], fp->layers[k]->output_size[1],
        fp->layers[k]->output_size[2]);
  } else {
    if (fp->layers[k]->type == FFN) {
      coeffs_from_previous_layer<<<num_out_neurons_last_layer, num_threads>>>(
          *linf_coeff, *lsup_coeff, *uinf_coeff, *usup_coeff, *linf_coeff_tmp,
          *lsup_coeff_tmp, *uinf_coeff_tmp, *usup_coeff_tmp,
          fp->layers[k]->coeffs, num_out_neurons_current_layer,
          num_in_neurons_current_layer);

      csts_from_previous_layer<<<num_out_neurons_last_layer, num_threads>>>(
          *linf_coeff, *lsup_coeff, *uinf_coeff, *usup_coeff, *linf_cst,
          *lsup_cst, *uinf_cst, *usup_cst, *linf_cst_tmp, *lsup_cst_tmp,
          *uinf_cst_tmp, *usup_cst_tmp, fp->layers[k]->csts,
          num_out_neurons_current_layer);
    } else {
      lcoeffs_from_previous_layer_pool<<<num_out_neurons_last_layer,
                                         fp->layers[k]->output_size[2]>>>(
          *linf_coeff, *lsup_coeff, *linf_coeff_tmp, *lsup_coeff_tmp,
          fp->layers[k]->pool_lcoeffs, fp->layers[k]->pool_ucoeffs,
          fp->layers[k]->output_size[0], fp->layers[k]->output_size[1],
          fp->layers[k]->output_size[2], fp->layers[k]->input_size[0],
          fp->layers[k]->input_size[1], fp->layers[k]->filter_size[0],
          fp->layers[k]->filter_size[1], fp->layers[k]->strides[0],
          fp->layers[k]->strides[1], fp->layers[k]->pad[0],
          fp->layers[k]->pad[1]);
      ucoeffs_from_previous_layer_pool<<<num_out_neurons_last_layer,
                                         fp->layers[k]->output_size[2]>>>(
      ...
      lcsts_from_input_poly<<<num_out_neurons_last_layer, 1>>>(
          ...
          *lsup_cst_tmp, fp->layers[k]->pool_lcsts, fp->layers[k]->pool_ucsts,
      ...
      ucsts_from_input_poly<<<num_out_neurons_last_layer, 1>>>(
          ...
          *usup_cst_tmp, fp->layers[k]->pool_lcsts, fp->layers[k]->pool_ucsts,
          fp->layers[k]->lb_array, fp->layers[k]->ub_array,
          num_out_neurons_current_layer);
    }
  }
    ...
    if (fp->layers[k]->type == RESIDUAL) {
      ...
      while (iter != common_predecessor) {
        update_state_using_predecessor_layer(
            fp, &linf_coeff, &lsup_coeff, &linf_cst, &lsup_cst, &uinf_coeff,
      ...
      while (iter != common_predecessor) {
        update_state_using_predecessor_layer(
            fp, &copy_linf_coeff, &copy_lsup_coeff, &copy_linf_cst,
...
void update_state_using_predecessor_layer_sparse(
    ...
    float_type **linf_cst_tmp, float_type **lsup_cst_tmp,
    float_type **uinf_coeff_tmp, float_type **usup_coeff_tmp,
    float_type **uinf_cst_tmp, float_type **usup_cst_tmp, const size_t layerno,
    const size_t k, const bool use_area_heuristic,
  ...
  const size_t missing_length =
      new_length_x * new_length_y * fp->layers[k]->input_size[2];
  ...
  if (fp->layers[k]->type == CONV) {
    if (fp->layers[k]->input_size[0] * fp->layers[k]->input_size[1] *
      ...
      if (fp->layers[k]->input_size[1] * fp->layers[k]->input_size[2] >
        ...
        coeffs_from_previous_layer_conv_sparse_x_filter_serial<<<
            ...
            *lsup_coeff_tmp, *uinf_coeff_tmp, *usup_coeff_tmp,
            ...
            length_y, shift_x, shift_y, new_length_x, new_length_y,
            fp->layers[k]->filter_size[0], fp->layers[k]->filter_size[1],
            fp->layers[k]->strides[0], fp->layers[k]->strides[1],
            fp->layers[k]->pad[0], fp->layers[k]->pad[1]);
      }
      ...
      coeffs_from_previous_layer_conv_sparse<<<
          ...
          *lsup_coeff_tmp, *uinf_coeff_tmp, *usup_coeff_tmp,
          ...
          fp->layers[k]->output_size[1], fp->layers[k]->output_size[2],
          fp->layers[k]->input_size[0], fp->layers[k]->input_size[1],
          fp->layers[k]->input_size[2], offset_x, offset_y, length_x, length_y,
          shift_x, shift_y, new_length_x, new_length_y,
          fp->layers[k]->filter_size[0], fp->layers[k]->filter_size[1],
          fp->layers[k]->strides[0], fp->layers[k]->strides[1],
          fp->layers[k]->pad[0], fp->layers[k]->pad[1]);
    }
    ...
    lcoeffs_from_previous_layer_pool_sparse<<<
        ...
        *linf_coeff, *lsup_coeff, *linf_coeff_tmp, *lsup_coeff_tmp,
        ...
        fp->layers[k]->output_size[0], fp->layers[k]->output_size[1],
        fp->layers[k]->output_size[2], fp->layers[k]->input_size[0],
        fp->layers[k]->input_size[1], offset_x, offset_y, length_x, length_y,
        shift_x, shift_y, new_length_x, new_length_y,
        fp->layers[k]->filter_size[0], fp->layers[k]->filter_size[1],
        fp->layers[k]->strides[0], fp->layers[k]->strides[1],
        fp->layers[k]->pad[0], fp->layers[k]->pad[1]);
    ...
    lcsts_from_input_poly_sparse<<<
        ...
        1>>>(*linf_coeff, *lsup_coeff, *linf_cst, *lsup_cst, *linf_cst_tmp,
             *lsup_cst_tmp, fp->layers[k]->pool_lcsts,
             fp->layers[k]->pool_ucsts, fp->layers[k]->lb_array,
             fp->layers[k]->ub_array, fp->layers[k]->output_size[0],
             fp->layers[k]->output_size[1], fp->layers[k]->output_size[2],
             offset_x, offset_y, length_x, length_y, shift_x, shift_y);
    ucsts_from_input_poly_sparse<<<
        ...
        1>>>(*uinf_coeff, *usup_coeff, *uinf_cst, *usup_cst, *uinf_cst_tmp,
             *usup_cst_tmp, fp->layers[k]->pool_lcsts,
             fp->layers[k]->pool_ucsts, fp->layers[k]->lb_array,
             fp->layers[k]->ub_array, fp->layers[k]->output_size[0],
             fp->layers[k]->output_size[1], fp->layers[k]->output_size[2],
             offset_x, offset_y, length_x, length_y, shift_x, shift_y);
...
__global__ void compute_c_array(
    float_type *__restrict__ c_arrays, const float_type *__restrict__ gradient,
    const float_type *__restrict__ linf_coeffs,
    const float_type *__restrict__ usup_coeffs, const int num_chunks,
    const int chunk_counter, const int output_size_x, const int output_size_y,
    const int output_size_z, const int offset_x, const int offset_y,
    const int length_x, const int length_y, const int shift_x,
    const int shift_y) {
  ...
  while (out_z < output_size_z) {
    for (int out_x = min_x; out_x < max_x; out_x++) {
      for (int out_y = min_y; out_y < max_y; out_y++) {
        ...
        const int i = (out_x + min_out_x) * output_size_y * output_size_z +
                      (out_y + min_out_y) * output_size_z + out_z;
        const int global_out =
            global_n * output_size_x * output_size_y * output_size_z + i;
...
__global__ void reduce_c_compute_xval(float_type *__restrict__ xval,
                                      const float_type *__restrict__ c_arrays,
                                      const float_type *__restrict__ input_inf,
                                      const float_type *__restrict__ input_sup,
                                      const int num_chunks,
                                      const int chunk_counter,
                                      const int num_neurons_last_layer,
...
__global__ void compute_adv(float_type *__restrict__ adv,
                            const float_type *__restrict__ gradient,
                            const float_type *__restrict__ xval,
                            const float_type *__restrict__ linf_coeffs,
                            const float_type *__restrict__ usup_coeffs,
                            const float_type *__restrict__ linf_csts,
                            const float_type *__restrict__ usup_csts,
                            const int num_chunks, const int chunk_counter,
                            const int output_size_x, const int output_size_y,
                            const int output_size_z, const int offset_x,
                            const int offset_y, const int length_x,
                            const int length_y, const int shift_x,
                            const int shift_y) {
  const int last_x = blockIdx.x;
  const int last_y = blockIdx.y;
  const int last_z = blockIdx.z;
...
void update_state_using_previous_layers_sparse(
  ...
  if (retain_training_data) {
    ...
    compute_c_array<<<dim3(fp->layers[layerno]->output_size[0],
                           fp->layers[layerno]->output_size[1],
                           fp->layers[layerno]->output_size[2] / num_chunks),
                      num_threads>>>(
        c_array, gradient_dev, linf_coeff, usup_coeff, num_chunks,
    ...
    compute_adv<<<dim3(fp->layers[layerno]->output_size[0],
                       fp->layers[layerno]->output_size[1],
                       fp->layers[layerno]->output_size[2] / num_chunks),
                  num_threads>>>(
        adv, gradient_dev, xval, linf_coeff, usup_coeff, linf_cst, usup_cst,
        num_chunks, chunk_counter, input_size_x, input_size_y, input_size_z,
        offset_x, offset_y, length_x, length_y, shift_x, shift_y);
...
void update_state_using_previous_layers_sparse_full(
    elina_manager_t *man, fppoly_t *fp, const size_t layerno,
    const bool use_area_heuristic, const bool retain_training_data,
    const float_type *gradient) {
  ...
  for (size_t chunk_counter = 0; chunk_counter < num_chunks; chunk_counter++) {
    update_state_using_previous_layers_sparse(
        ...
        use_area_heuristic, retain_training_data, gradient);
  }
}

void ffn_handle_intermediate_layer(
    elina_manager_t *man, elina_abstract0_t *element,
    const float_type *const *weights, const float_type *cst,
    ...
    const bool use_area_heuristic, fnn_op OP) {
  ...
  if (OP == MUL || OP == SUB1 || OP == SUB2) {
    float_type **dense_weights =
        (float_type **)malloc(num_in_neurons * sizeof(float_type *));

    for (size_t j = 0; j < num_in_neurons; j++) {
      dense_weights[j] =
          (float_type *)calloc(num_in_neurons, sizeof(float_type));
    }
    ...
    if (OP == MUL) {
      ...
      layer_create_dense_exprs(coeffs, csts, dense_weights, cst, num_in_neurons,
                               num_in_neurons);
    } else if (OP == SUB2) {
      float_type *dense_cst =
          (float_type *)calloc(num_in_neurons, sizeof(float_type));
    ...
    layer_create_dense_exprs(coeffs, csts, weights, cst, num_out_neurons,
                             num_in_neurons);
  }
...
void ffn_handle_intermediate_sub_layer(
    ...
    const bool use_area_heuristic) {
  if (is_minuend == true) {
    ffn_handle_intermediate_layer(man, element, nullptr, cst, num_in_neurons,
                                  num_in_neurons, predecessors, NONE, true,
                                  use_area_heuristic, SUB1);
  } else {
    ffn_handle_intermediate_layer(man, element, nullptr, cst, num_in_neurons,
                                  num_in_neurons, predecessors, NONE, true,
                                  use_area_heuristic, SUB2);
  }
}
...
void ffn_handle_intermediate_mul_layer(elina_manager_t *man,
                                       ...
                                       size_t *predecessors,
                                       const bool use_area_heuristic) {
  ffn_handle_intermediate_layer(man, element, nullptr, bias, num_in_neurons,
                                num_in_neurons, predecessors, NONE, true,
                                use_area_heuristic, MUL);
}

void ffn_handle_last_layer(elina_manager_t *man, elina_abstract0_t *element,
                           const float_type *const *weights,
                           const float_type *bias, const size_t num_out_neurons,
...
void update_state_using_predecessor_layer_lower_half(
    fppoly_t *fp, float_type **linf_coeff, float_type **lsup_coeff,
    float_type **linf_cst, float_type **lsup_cst, float_type **linf_coeff_tmp,
    float_type **lsup_coeff_tmp, float_type **linf_cst_tmp,
    float_type **lsup_cst_tmp, const size_t k, const bool use_area_heuristic,
    const size_t num_out_neurons_last_layer) {
  const size_t num_out_neurons_current_layer = fp->layers[k]->num_out_neurons;
  const size_t num_in_neurons_current_layer = fp->layers[k]->num_in_neurons;
  ...
  *lsup_coeff_tmp = malloc_device<float_type>(num_out_neurons_last_layer *
  ...
  if (fp->layers[k]->type == CONV) {
    ...
    csts_from_previous_layer_conv<<<num_out_neurons_last_layer,
                                    fp->layers[k]->output_size[2]>>>(
        *linf_coeff, *lsup_coeff, *linf_cst, *lsup_cst, *linf_cst_tmp,
        *lsup_cst_tmp, fp->layers[k]->filter_bias,
        fp->layers[k]->output_size[0], fp->layers[k]->output_size[1],
        fp->layers[k]->output_size[2]);
    ...
    if (fp->layers[k]->type == FFN) {
      ...
      csts_from_previous_layer<<<num_out_neurons_last_layer, num_threads>>>(
          *linf_coeff, *lsup_coeff, *linf_cst, *lsup_cst, *linf_cst_tmp,
          *lsup_cst_tmp, fp->layers[k]->csts, num_out_neurons_current_layer);
    } else {
      lcoeffs_from_previous_layer_pool<<<num_out_neurons_last_layer,
                                         fp->layers[k]->output_size[2]>>>(
          *linf_coeff, *lsup_coeff, *linf_coeff_tmp, *lsup_coeff_tmp,
    ...
    if (fp->layers[k]->type == RESIDUAL) {
      ...
      while (iter != common_predecessor) {
        update_state_using_predecessor_layer_lower_half(
            fp, &linf_coeff, &lsup_coeff, &linf_cst, &lsup_cst, &linf_coeff_tmp,
            &lsup_coeff_tmp, &linf_cst_tmp, &lsup_cst_tmp, iter,
            use_area_heuristic, num_out_neurons_last_layer);
      ...
      update_state_using_predecessor_layer_lower_half(
          fp, &linf_coeff, &lsup_coeff, &linf_cst, &lsup_cst, &linf_coeff_tmp,
          &lsup_coeff_tmp, &linf_cst_tmp, &lsup_cst_tmp, k, use_area_heuristic,
          num_out_neurons_last_layer);
...
void layer_create_sparse_exprs(fppoly_t *const fp,
                               ...
                               const size_t *filter_size, const size_t *strides,
...
void conv_handle_first_layer(elina_manager_t *man, elina_abstract0_t *element,
  ...
  layer_create_sparse_exprs(fp, filter_weights, filter_bias, input_size,
                            filter_size, strides, output_size, pad_top,
                            pad_left, has_bias, RELU);
...
void conv_handle_intermediate_layer(
    ...
    size_t *predecessors, const activation_type_t activation,
    const bool use_area_heuristic, const bool retain_training_data,
    const float_type *gradient) {
  fppoly_t *const fp = fppoly_of_abstract0(element);
  ...
  layer_create_sparse_exprs(fp, filter_weights, filter_bias, input_size,
                            filter_size, strides, output_size, pad_top,
                            pad_left, has_bias, activation);

  fp->layers[fp->numlayers - 1]->predecessors = predecessors;

  update_state_using_previous_layers_sparse_full(
...
void conv_handle_intermediate_relu_layer(
  ...
  conv_handle_intermediate_layer(
      man, element, filter_weights, filter_bias, input_size, filter_size,
      num_filters, strides, output_size, pad_top, pad_left, has_bias,
      predecessors, RELU, use_area_heuristic, retain_training_data, gradient);
}
...
void conv_handle_intermediate_affine_layer(
  ...
  conv_handle_intermediate_layer(
      man, element, filter_weights, filter_bias, input_size, filter_size,
      num_filters, strides, output_size, pad_top, pad_left, has_bias,
      predecessors, NONE, use_area_heuristic, retain_training_data, gradient);
}

void pool_add_layer(fppoly_t *const fp, const size_t num_out_neurons,
                    const size_t num_in_neurons, const layertype_t type,
                    const activation_type_t activation) {
  layer_t *layer = (layer_t *)malloc(sizeof(layer_t));
...
size_t handle_pool_layer(elina_manager_t *man, elina_abstract0_t *element,
                         ...
                         const size_t *strides, const size_t dimensionality,
                         const size_t pad_top, const size_t pad_left,
                         const size_t *output_size, size_t *predecessors,
                         const bool is_maxpool) {
  ...
  fp->layers[fp->numlayers - 1]->pool_lcoeffs =
      malloc_device<float_type>(num_out_neurons * pool_size[0] * pool_size[1]);
  fp->layers[fp->numlayers - 1]->pool_ucoeffs =
      malloc_device<float_type>(num_out_neurons * pool_size[0] * pool_size[1]);

  float_type *lb_in_array =
      (float_type *)malloc(num_in_neurons * sizeof(float_type));
  float_type *ub_in_array =
      (float_type *)malloc(num_in_neurons * sizeof(float_type));

  float_type *lb_out_array =
      (float_type *)malloc(num_out_neurons * sizeof(float_type));
  float_type *ub_out_array =
      (float_type *)malloc(num_out_neurons * sizeof(float_type));

  copy_to_host(lb_in_array, fp->layers[fp->numlayers - 2]->lb_array,
               num_in_neurons);
  copy_to_host(ub_in_array, fp->layers[fp->numlayers - 2]->ub_array,
               num_in_neurons);

  float_type *pool_lcoeffs_host = (float_type *)calloc(
      num_out_neurons * pool_size[0] * pool_size[1], sizeof(float_type));
  float_type *pool_ucoeffs_host = (float_type *)calloc(
      num_out_neurons * pool_size[0] * pool_size[1], sizeof(float_type));

  float_type *pool_lcsts_host =
      (float_type *)malloc(num_out_neurons * sizeof(float_type));
  float_type *pool_ucsts_host =
      (float_type *)malloc(num_out_neurons * sizeof(float_type));

  float_type *inf =
      (float_type *)calloc(pool_size[0] * pool_size[1], sizeof(float_type));
  float_type *sup =
      (float_type *)calloc(pool_size[0] * pool_size[1], sizeof(float_type));
  ...
  for (size_t out_x = 0; out_x < output_size[0]; out_x++) {
    for (size_t out_y = 0; out_y < output_size[1]; out_y++) {
      for (size_t out_z = 0; out_z < output_size[2]; out_z++) {
        size_t out_pos = out_x * output_size[1] * output_size[2] +
                         out_y * output_size[2] + out_z;

        size_t inp_x = out_x * pool_size[0];
        size_t inp_y = out_y * pool_size[1];
        size_t inp_z = out_z;
        size_t inp_pos = inp_x * input_size[1] * input_size[2] +
                         inp_y * input_size[2] + inp_z;
        ...
        for (size_t x_shift = 0; x_shift < pool_size[0]; x_shift++) {
          for (size_t y_shift = 0; y_shift < pool_size[1]; y_shift++) {
            ...
            size_t pool_cur_dim = inp_pos +
                                  x_shift * input_size[1] * input_size[2] +
  ...
  copy_to_device(fp->layers[fp->numlayers - 1]->lb_array, lb_out_array,
                 num_out_neurons);
  copy_to_device(fp->layers[fp->numlayers - 1]->ub_array, ub_out_array,
                 num_out_neurons);

  copy_to_device(fp->layers[fp->numlayers - 1]->pool_lcoeffs, pool_lcoeffs_host,
                 num_out_neurons * pool_size[0] * pool_size[1]);
  copy_to_device(fp->layers[fp->numlayers - 1]->pool_ucoeffs, pool_ucoeffs_host,
                 num_out_neurons * pool_size[0] * pool_size[1]);

  copy_to_device(fp->layers[fp->numlayers - 1]->pool_lcsts, pool_lcsts_host,
                 num_out_neurons);
  copy_to_device(fp->layers[fp->numlayers - 1]->pool_ucsts, pool_ucsts_host,
                 num_out_neurons);
...
void handle_residual_layer(elina_manager_t *man, elina_abstract0_t *element,
  ...
  update_state_using_previous_layers_sparse_full(
      man, fp, fp->numlayers - 1, use_area_heuristic, false, nullptr);
}
