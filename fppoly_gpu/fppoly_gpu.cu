...
__device__ __host__ expr_t *
replace_input_poly_cons_in_lexpr(expr_t *expr, double *input_inf,
...
__device__ __host__ expr_t *
replace_input_poly_cons_in_uexpr(expr_t *expr, double *input_inf,
                                 double *input_sup, expr_t **input_lexpr,
                                 expr_t **input_uexpr) {
  double tmp1, tmp2;
  expr_t *res;
...
__device__ __host__ double compute_lb_from_expr(expr_t *expr, double *input_inf,
                                                double *input_sup,
                                                expr_t **input_lexpr,
                                                expr_t **input_uexpr) {
  // printf("start\n");
  ...
  if ((input_lexpr != nullptr) && (input_uexpr != nullptr)) {
    expr = replace_input_poly_cons_in_lexpr(expr, input_inf, input_sup,
...
__device__ __host__ double compute_ub_from_expr(expr_t *expr, double *input_inf,
                                                double *input_sup,
                                                expr_t **input_lexpr,
                                                expr_t **input_uexpr) {
  if ((input_lexpr != nullptr) && (input_uexpr != nullptr)) {
    expr = replace_input_poly_cons_in_uexpr(expr, input_inf, input_sup,
                                            input_lexpr, input_uexpr);
  }
...
void layer_create_exprs(neuron_t **neurons, const double **weights,
...
void layer_compute_bounds_from_exprs(neuron_t **neurons, double *input_inf,
  ...
  for (size_t i = 0; i < size; i++) {
    neuron_t *neuron = neurons[i];
    neuron->lb = compute_lb_from_expr(neuron->expr, input_inf, input_sup,
                                      input_lexpr, input_uexpr);
    neuron->ub = compute_ub_from_expr(neuron->expr, input_inf, input_sup,
                                      input_lexpr, input_uexpr);
  }
}
...
void ffn_handle_first_layer(elina_manager_t *man, elina_abstract0_t *abs,
  ...
  layer_create_exprs(neurons, weights, bias, size, num_pixels);
...
__device__ __host__ void lexpr_replace_relu_bounds(expr_t **expr_ptr,
                                                   neuron_t **neurons) {
...
__device__ __host__ void uexpr_replace_relu_bounds(expr_t **expr_ptr,
                                                   neuron_t **neurons) {
  expr_t *expr = *expr_ptr;
  const size_t num_neurons = expr->size;
...
__device__ __host__ void lexpr_replace_maxpool_bounds(expr_t **expr_ptr,
                                                      neuron_t **neurons) {
  // printf("begin\n");
  // fflush(stdout);
  expr_t *expr = *expr_ptr;
...
__device__ __host__ void uexpr_replace_maxpool_bounds(expr_t **expr_ptr,
                                                      neuron_t **neurons) {
  expr_t *expr = *expr_ptr;
  const size_t num_neurons = expr->size;
  expr_t *res;
...
__device__ __host__ void expr_from_previous_layer(expr_t **expr_ptr,
                                                  neuron_t **neurons) {
  expr_t *expr = *expr_ptr;
  ...
  if (neurons[k]->expr->size == 0) {
    ...
    res =
        multiply_expr(neurons[k]->expr, expr->inf_coeff[0], expr->sup_coeff[0]);
  }
    ...
    for (int k = layerno - 1; k >= 0; k--) {
      ...
      if (fp->layers[k]->type != MAXPOOL) {
        if (fp->layers[k]->activation == RELU) {
          lexpr_replace_relu_bounds(lexpr_ptr, aux_neurons);
          uexpr_replace_relu_bounds(uexpr_ptr, aux_neurons);
        }

        expr_from_previous_layer(lexpr_ptr, aux_neurons);
        expr_from_previous_layer(uexpr_ptr, aux_neurons);
      } else {
        lexpr_replace_maxpool_bounds(lexpr_ptr, aux_neurons);
        uexpr_replace_maxpool_bounds(uexpr_ptr, aux_neurons);
      }
    }

    out_neurons[i]->lb = compute_lb_from_expr(
        lexpr, fp->input_inf, fp->input_sup, fp->input_lexpr, fp->input_uexpr);
    out_neurons[i]->ub = compute_ub_from_expr(
        lexpr, fp->input_inf, fp->input_sup, fp->input_lexpr, fp->input_uexpr);
...
void ffn_handle_intermediate_layer(elina_manager_t *man,
  ...
  layer_create_exprs(out_neurons, weights, bias, num_out_neurons,
                     num_in_neurons);
...
double get_lb_using_previous_layers(elina_manager_t *man,
  ...
  return compute_lb_from_expr(lexpr, fp->input_inf, fp->input_sup,
...
void conv_handle_first_layer(elina_manager_t *man, elina_abstract0_t *abs,
  ...
  const size_t size = output_size[0] * output_size[1] * output_size[2];

  fppoly_internal_t *pr =
      fppoly_init_from_manager(man, ELINA_FUNID_ASSIGN_LINEXPR_ARRAY);
  fppoly_t *res = fppoly_of_abstract0(abs);
  fppoly_alloc_first_layer(res, size, num_pixels, CONV, RELU);
  ...
  for (out_x = 0; out_x < output_size[0]; out_x++) {
    for (out_y = 0; out_y < output_size[1]; out_y++) {
      for (out_z = 0; out_z < output_size[2]; out_z++) {
        ...
        neurons[mat_x]->lb = compute_lb_from_expr(
            neurons[mat_x]->expr, res->input_inf, res->input_sup,
            res->input_lexpr, res->input_uexpr);
        neurons[mat_x]->ub = compute_ub_from_expr(
            neurons[mat_x]->expr, res->input_inf, res->input_sup,
            res->input_lexpr, res->input_uexpr);
  ...
  for (out_pos = 0; out_pos < num_out_neurons; out_pos++) {
    ...
    for (x_shift = 0; x_shift < pool_size[0]; x_shift++) {
      for (y_shift = 0; y_shift < pool_size[1]; y_shift++) {
        ...
        const double lb = -fp->layers[numlayers - 1]->neurons[pool_cur_dim]->lb;
        const double ub = fp->layers[numlayers - 1]->neurons[pool_cur_dim]->ub;
...
elina_linexpr0_t *get_expr_for_output_neuron(elina_manager_t *man,
  ...
  if ((fp->input_lexpr != nullptr) && (fp->input_uexpr != nullptr)) {
    if (is_lower) {
      expr = replace_input_poly_cons_in_lexpr(
          expr, fp->input_inf, fp->input_sup, fp->input_lexpr, fp->input_uexpr);
    } else {
      expr = replace_input_poly_cons_in_uexpr(
          expr, fp->input_inf, fp->input_sup, fp->input_lexpr, fp->input_uexpr);
    }
  }
