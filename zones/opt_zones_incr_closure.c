...
void floyd_warshall_incr_vector(double *m, unsigned short int n,
  ...
  // store v-th column in array to improve cache performance and enable
  // vectorization
  for (i = 0; i < n; i++) {
    tmpv[i] = m[n * i + v1];
  }
...
bool incr_closure_dense(opt_zones_mat_t *oz, unsigned short int dim,
                        unsigned short int v) {
  double *m = oz->mat;
...
void floyd_warshall_incr_scalar(double *m, unsigned short int n,
                                unsigned short int v) {
  unsigned short int i, j, k;
  // load the v-th row
  unsigned short int v1 = v + 1;
...
bool incr_closure_dense_scalar(opt_zones_mat_t *oz, unsigned short int dim,
                               unsigned short int v) {
  double *m = oz->mat;
  unsigned short int n = dim + 1;
...
bool incr_closure_comp_sparse(opt_zones_mat_t *oz, unsigned short int dim,
                              unsigned short int v) {
  ...
  if (cl != NULL) {
    ...
    unsigned short int *index1 =
        (unsigned short int *)calloc(comp_size + 1, sizeof(unsigned short int));
