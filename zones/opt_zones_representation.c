...
/********* Allocate a new zones abstract element by setting the appropriate
 * matrix **************/
...
opt_zones_t *opt_zones_copy(elina_manager_t *man, opt_zones_t *o) {
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_COPY, 0);
...
void opt_zones_free(elina_manager_t *man, opt_zones_t *o) {
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_FREE, 0);
...
opt_zones_t *opt_zones_bottom(elina_manager_t *man, unsigned short int intdim,
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_BOTTOM, 0);
...
opt_zones_t *opt_zones_top(elina_manager_t *man, unsigned short int intdim,
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_TOP, 0);
...
void opt_zones_minimize(elina_manager_t *man, opt_zones_t *o) {
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_MINIMIZE, 0);
...
void opt_zones_canonicalize(elina_manager_t *man, opt_zones_t *o) {
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_CANONICALIZE, 0);
  elina_manager_raise_exception(man, ELINA_EXC_NOT_IMPLEMENTED, pr->funid,
                                "not implemented");
}
...
int opt_zones_hash(elina_manager_t *man, opt_zones_t *o) {
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_HASH, 0);
...
void opt_zones_approximate(elina_manager_t *man, opt_zones_t *o,
                           int algorithm) {
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_APPROXIMATE, 0);
  elina_manager_raise_exception(man, ELINA_EXC_NOT_IMPLEMENTED, pr->funid,
                                "not implemented");
}
/****
...
opt_zones_t *opt_zones_closure(elina_manager_t *man, bool destructive,
                               opt_zones_t *o) {
  opt_zones_internal_t *pr =
      (opt_zones_internal_t *)opt_zones_init_from_manager(
...
elina_manager_t *opt_zones_manager_alloc(void) {
  ...
  if (!elina_fpu_init()) {
    fprintf(stderr,
            "opt_zones_manager_alloc cannot change the FPU rounding mode\n");
  }
  ...
  man->funptr[ELINA_FUNID_IS_DIMENSION_UNCONSTRAINED] =
      &opt_zones_is_dimension_unconstrained;
  ...
  // man->funptr[ELINA_FUNID_TO_GENERATOR_ARRAY] =
  // &opt_zones_to_generator_array;
  ...
  // &opt_zones_assign_linexpr_array;
  // man->funptr[ELINA_FUNID_SUBSTITUTE_LINEXPR_ARRAY] =
  // &opt_zones_substitute_linexpr_array;
  man->funptr[ELINA_FUNID_ASSIGN_TEXPR_ARRAY] = &opt_zones_assign_texpr_array;
  // man->funptr[ELINA_FUNID_SUBSTITUTE_TEXPR_ARRAY] =
  // &opt_zones_substitute_texpr_array;
