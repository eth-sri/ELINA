...
opt_zones_t *opt_zones_forget_array(elina_manager_t *man, bool destructive,
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_FORGET_ARRAY, 0);
  ...
  if (!o->closed && !o->m)
    ...
    opt_zones_mat_t *oz = o->closed ? o->closed : o->m;
    if (!destructive)
      oz = opt_zones_mat_copy(oz, o->dim);
#if defined(TIMING)
    start_timing();
#endif
    ...
    if (o->closed) {
      ...
      if (project)
        ...
        r = opt_zones_set_mat(pr, o, NULL, oz, destructive);
    } else {
...
opt_zones_t *opt_zones_add_dimensions(elina_manager_t *man, bool destructive,
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_ADD_DIMENSIONS, 0);
  ...
  if (!src)
    ...
    opt_zones_mat_addrem_dimensions(dst, src, dimchange->dim, nb, 1, o->dim,
                                    true);
...
opt_zones_t *opt_zones_remove_dimensions(elina_manager_t *man, bool destructive,
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_REMOVE_DIMENSIONS, 0);
...
opt_zones_t *opt_zones_expand(elina_manager_t *man, bool destructive,
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_EXPAND, 0);
  opt_zones_mat_t *src = o->closed ? o->closed : o->m;
  size_t i, j;
