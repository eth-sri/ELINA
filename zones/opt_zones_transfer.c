...
opt_zones_t *opt_zones_assign_texpr_array(elina_manager_t *man,
  ...
  opt_zones_t *r = elina_generic_assign_texpr_array(man, destructive, o, tdim,
                                                    texpr, size, dest);
...
opt_zones_t *opt_zones_meet_lincons_array(elina_manager_t *man,
  ...
  opt_zones_internal_t *pr = opt_zones_init_from_manager(
      man, ELINA_FUNID_MEET_LINCONS_ARRAY, 2 * (o->dim + 8));
  if (!o->closed && !o->m)
    /* definitively empty */
    ...
    start_timing();
#endif
    bool res = opt_zones_mat_add_lincons(pr, oz, o->intdim, o->dim, array,
    ...
    if (res) {
      /* empty */
      if (!destructive)
        opt_zones_mat_free(oz);
      return opt_zones_set_mat(pr, o, NULL, NULL, destructive);
    } else {
      /* exact if zonal constraints & no conversion error */
      if (zone_num_incomplete || !exact)
...
opt_zones_t *opt_zones_meet_tcons_array(elina_manager_t *man, bool destructive,
  ...
  opt_zones_t *r = elina_generic_meet_intlinearize_tcons_array(
      man, destructive, o, array, ELINA_SCALAR_DOUBLE, ELINA_LINEXPR_INTLINEAR,
