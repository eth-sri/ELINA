...
bool opt_zones_is_bottom(elina_manager_t *man, opt_zones_t *o) {
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_IS_BOTTOM, 0);
  ...
  if (o->closed) {
    ...
    if (zone_num_incomplete || o->intdim) {
      zones_flag_incomplete;
    }
...
bool opt_zones_is_top(elina_manager_t *man, opt_zones_t *o) {
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_IS_TOP, 0);
  int i, j;
...
bool opt_zones_is_leq(elina_manager_t *man, opt_zones_t *o1, opt_zones_t *o2) {
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_IS_LEQ, 0);
  ...
  if (!o1->closed && !o1->m) {
    ...
    if (o1->closed) {
      /* a1 not empty on Q */
      if (zone_num_incomplete || o1->intdim) {
        zones_flag_incomplete;
      }
      return false;
    } else {
...
bool opt_zones_is_eq(elina_manager_t *man, opt_zones_t *o1, opt_zones_t *o2) {
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_IS_EQ, 0);
  if ((o1->dim != o2->dim) || (o1->intdim != o2->intdim))
    return false;
  if (pr->funopt->algorithm >= 0) {
...
elina_interval_t **opt_zones_to_box(elina_manager_t *man, opt_zones_t *o) {
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_TO_BOX, 0);
    ...
    if (!oz->is_dense) {
      ...
      while (cl != NULL) {
        ...
        for (i = 0; i < comp_size; i++) {
          int i1 = c->num;
          zones_interval_of_bounds(in[i1], m[n * (i1 + 1)], m[i1 + 1]);
      ...
      for (i = 1; i < n; i++) {
        zones_interval_of_bounds(in[i - 1], m[n * i], m[i]);
      }
    }
    man->result.flag_exact = false;
    if (!o->closed)
      zone_flag_algo;
    else if (zone_num_incomplete || o->intdim)
      zones_flag_incomplete;
    else if (pr->conv)
      zone_flag_conv;
  }
...
elina_lincons0_array_t opt_zones_to_lincons_array(elina_manager_t *man,
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_TO_LINCONS_ARRAY, 0);
...
elina_interval_t *opt_zones_bound_dimension(elina_manager_t *man,
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_BOUND_DIMENSION, 0);
...
bool opt_zones_is_dimension_unconstrained(elina_manager_t *man, opt_zones_t *o,
                                          elina_dim_t dim) {
  opt_zones_internal_t *pr = opt_zones_init_from_manager(
      man, ELINA_FUNID_IS_DIMENSION_UNCONSTRAINED, 0);
  if (dim >= o->dim) {
    return false;
  }
