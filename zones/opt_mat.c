...
array_comp_list_t *extract_comps_zones(double *m, unsigned short int dim) {
  ...
  unsigned short int *owner =
      (unsigned short int *)calloc(dim, sizeof(unsigned short int));
...
void opt_zones_comp_copy(double *dest, double *src, comp_list_t *cl,
...
void opt_zones_sparse_copy(opt_zones_mat_t *dst_mat, opt_zones_mat_t *src_mat,
                           unsigned short int dim) {
  dst_mat->ti = false;
...
opt_zones_mat_t *opt_zones_mat_copy(opt_zones_mat_t *oz,
                                    unsigned short int dim) {
  if (!oz) {
    return NULL;
  }
...
*****/
void convert_to_dense_zones(opt_zones_mat_t *oz, unsigned short int dim,
                            bool flag) {
  double *src = oz->mat;
  src[0] = 0;
  unsigned short int n = dim + 1;
...
bool check_is_leq_zones(double *m1, double *m2, unsigned short int i,
                        unsigned short int j, unsigned short int dim) {
  unsigned short int n = dim + 1;
  int ind = n * i + j;
  int ind1 = n * i;
  int ind2 = j;
...
bool is_lequal_zones_mat(opt_zones_mat_t *oz1, opt_zones_mat_t *oz2,
                         unsigned short int dim) {
  double *m1 = oz1->mat;
  double *m2 = oz2->mat;
  unsigned short int n = dim + 1;
  ...
  if (!oz1->is_dense && !oz2->is_dense) {
    /****
            If both oo1 and oo2 are decomposed type, apply decomposed type
    operator Compute intersection of components and check if oo1 is <= oo2 in
    lattice as follows:
            1. Elements corresponding to components in oo2 not in intersection
    should all be oo
            2. Elements cooresponding to components in intersection should be
    equal
    ****/
...
bool check_is_eq_zones(double *m1, double *m2, unsigned short int i,
                       unsigned short int j, unsigned short int dim) {
  unsigned short int n = dim + 1;
  int ind = n * i + j;
  int ind1 = n * i;
...
bool is_equal_zones_mat(opt_zones_mat_t *oz1, opt_zones_mat_t *oz2,
                        unsigned short int dim) {
  double *m1 = oz1->mat;
  double *m2 = oz2->mat;
  unsigned short int n = dim + 1;
    ...
    ******/
    if (!is_equal_array_comp_list(oz1->acl, oz2->acl, dim)) {
      /***
              ...
              Compute intersection of components and check the elements for
      equality as follows:
              1. Elements corresponding to components in oo1 not in intersection
      should all be oo
              2. Elements corresponding to components in oo2 not in intersection
      should all be oo
              3. Elements cooresponding to components in intersection should be
      equal
      ****/
      array_comp_list_t *acl =
          intersection_array_comp_list(oz1->acl, oz2->acl, dim);
      /****
              Check 1
      ****/
      ...
      *******/
      ...
      /****
              If we come here we know that both oo1 and oo2 contain the same set
...
void meet_zones_mat(opt_zones_mat_t *oz, opt_zones_mat_t *oz1,
  ...
  if (!oz1->is_dense && !oz2->is_dense) {
    /*****
            If both oo1 and oo2 are decomposed type, then apply the decomposed
    type operator,
            1. Compute union of corresponding sets of independent components.
            2. Initialize oo1 elements corresponding to components in union not
    in oo1
            3. Initialize oo2 elements corresponding to components in union not
    in oo2
            4. Compute meet of oo1 and oo2 by operating on elements
...
bool is_equal_comp_zones(opt_zones_mat_t *oz1, opt_zones_mat_t *oz2,
                         unsigned short int *arr_map1,
                         unsigned short int *arr_map2, comp_list_t *cl,
                         unsigned short int dim) {
  double *m1 = oz1->mat;
  double *m2 = oz2->mat;
  unsigned short int comp_size = cl->size;
...
void sparse_join_zones_mat(opt_zones_mat_t *oz, opt_zones_mat_t *oz1,
                           opt_zones_mat_t *oz2, unsigned short int dim,
                           bool destructive) {
  ...
  if (count >= 1) {
    ...
    if (count > 1) {
      ...
      // unsigned short int * index = (unsigned short int *)calloc(2*(2*dim +
      // 1),sizeof(unsigned short int)); double * temp = (double
      // *)malloc(2*dim*sizeof(double));
...
void join_zones_mat(opt_zones_mat_t *oz, opt_zones_mat_t *oz1,
  ...
  if ((!oz1->is_dense) || (!oz2->is_dense)) {
    /******
            If either oo1 or oo2 is decomposed type, apply the decomposed type
    operator
            1. Compute the intersection of corresponding sets of independent
    components.
            2. Compute join of oo1 and oo2 by operating on elements
    corresponding to intersection.
    ******/
    array_comp_list_t *temp = oz1->acl;
    /*****
            Step 1
...
void set_infinity_widening_zones(double *m, unsigned short int i,
                                 unsigned short int j, unsigned short int dim) {
  unsigned short int n = dim + 1;
  int ind = n * i + j;
  m[ind] = INFINITY;
...
void set_widening_zones(double *m, double *m1, double *m2, unsigned short int i,
                        unsigned short int j, unsigned short int dim,
                        int *count) {
  unsigned short int n = dim + 1;
  int ind = n * i + j;
...
void sparse_widening_zones_mat(opt_zones_mat_t *oz, opt_zones_mat_t *oz1,
                               opt_zones_mat_t *oz2, unsigned short int dim) {
  double *m = oz->mat;
  double *m1 = oz1->mat;
  double *m2 = oz2->mat;
...
void opt_zones_mat_addrem_dimensions(opt_zones_mat_t *dz, opt_zones_mat_t *sz,
                                     elina_dim_t *pos,
                                     unsigned short int nb_pos,
                                     unsigned short int mult,
                                     unsigned short int dim, bool add) {

  unsigned short int i, j;
  double *dst = dz->mat;
  double *src = sz->mat;
  unsigned short int *map =
      (unsigned short int *)calloc(dim + 1, sizeof(unsigned short int));
  ...
  if (nb_pos) {
    ...
    if (add) {
      ...
      ****/
      ...
      /******
              Because of add, the independent components are shifted in
      destination matrix Build Map of Independent Components for add in
      destination matrix Map contains new positions of components in the
      destination matrix
      ******/
      ...
      for (int i = 0; i <= dim; i++) {
        ...
        while ((l < nb_pos) && (i == pos[l])) {
          /***
                  For expand mult can be greater than 1 in which case nb_pos
          will be 1
          ****/
          add_pos[ac] = p;
          p = p + mult;
      ...
      ****/
      ...
      /******
              Because of remove, the independent components are shifted in
      destination matrix Build Map of Independent Components for add in
      destination matrix Map contains new positions of components in the
      destination matrix
      ******/
...
void opt_zones_mat_permute(opt_zones_mat_t *dz, opt_zones_mat_t *sz,
                           unsigned short int dst_dim,
                           unsigned short int src_dim,
...
zone_expr zone_expr_of_linexpr(opt_zones_internal_t *pr, double *dst,
                               elina_linexpr0_t *e, unsigned short int intdim,
                               unsigned short int dim) {
...
#define CLASS_VAR(idx)                                                         \
  ...
  if (z.type == OPT_EMPTY)                                                     \
    continue;                                                                  \
  if ((dst[2 * idx + 2] == 0) && (dst[2 * idx + 3] == 0))                      \
    continue;                                                                  \
  if (z.type >= OPT_BINARY) {                                                  \
    z.type = OPT_OTHER;                                                        \
    continue;                                                                  \
  }                                                                            \
  CLASS_COEFF(idx, 1);                                                         \
...
#define COEFF(c, i)                                                            \
  ...
  if (opt_bounds_of_coeff(pr, dst + i, dst + i + 1, c))                        \
    z.type = OPT_EMPTY;                                                        \
  if (c.discr != ELINA_COEFF_SCALAR || !is_integer(dst[i]))                    \
    z.is_int = false;
...
bool opt_zones_mat_add_lincons(opt_zones_internal_t *pr, opt_zones_mat_t *oz,
                               unsigned short int intdim,
                               unsigned short int dim,
                               elina_lincons0_array_t *array, bool *exact,
                               bool *respect_closure) {
  double *m = oz->mat;
  unsigned short int i, j, k;
  int zi, zj;
  unsigned short int var_pending =
      0; /* delay incremental closure as long as possible */
  ...
  for (i = 0; i < array->size; i++) {
    ...
    case OPT_BINARY:
      ...
      /******
        ...
        3. If u.i and u.j are present in different components, then take the
      union of containing components.
    ...
    case OPT_OTHER: {
      ...
      *respect_closure = false; /* do not respect closure */
      ...
      if (Cb == INFINITY)
        ...
        /******
               Handle Independent Components in case Quadratic Number of Bounds
         are Created. Find the component containing each j and merge it with
         other components using strengthening, this creates over approximation.
         *****/

        // acl = oo->acl;
        for (j = 0; j < dim; j++) {
          // contribution by vj is positive
          if ((pr->tmp[2 * j + 2] <= -1) && (m[j + 1] != INFINITY)) {
          ...
          else if ((pr->tmp[2 * j + 3] <= -1) && (m[n * (j + 1)] != INFINITY)) {
            /* vj <= expr+vj <= max(expr) - max(-vj) */
        ...
        if ((pr->tmp[2 * Cj1 + 3] == -1) && (pr->tmp[2 * Cj1 + 2] == 1))
          zj = -1;
        else if ((pr->tmp[2 * Cj1 + 3] == 1) && (pr->tmp[2 * Cj1 + 2] == -1))
          zj = 1;
        else
          goto Cbrk;
        /****
              Handle the Independent Components in case Linear Number of Bounds
        are created. Find the component containing each Cj1 and merge it with
        other components using strengthening, this creates over approximation
        ...
        *****/
        ...
        if ((pr->tmp[2 * Cj2 + 3] == -1) && (pr->tmp[2 * Cj2 + 2] == 1))
          zj = -1;
        else if ((pr->tmp[2 * Cj2 + 3] == 1) && (pr->tmp[2 * Cj2 + 2] == -1))
          zj = 1;
        else
          goto Cbrk;
        /*****
    ...
    Cbrk:
      ...
      if (c == ELINA_CONS_EQ) {
        ...
        if (cb == INFINITY)
          ...
          /******
               Handle Independent Components in case Quadratic Number of Bounds
         are Created. Handling is similar to quadratic case above.
         *****/

          // acl = oo->acl;
          for (j = 0; j < dim; j++) {
            if ((pr->tmp[2 * j + 3] <= -1) && (m[j + 1] != INFINITY)) {
              tmpa = cb - m[j + 1];
              zj = -1;
            ...
            for (k = j + 1; k < dim; k++) {
              ...
              if ((pr->tmp[2 * k + 3] <= -1) && (m[k + 1] != INFINITY) &&
                  (zj == 1)) {
                tmpb = tmpa - m[k + 1];
                if (m[n * (k + 1) + j + 1] == INFINITY) {
                  m[n * (k + 1) + j + 1] = tmpb;
          ...
          if ((pr->tmp[2 * cj1 + 2] == -1) && (pr->tmp[2 * cj1 + 3] == 1))
            zj = 1;
          else if ((pr->tmp[2 * cj1 + 2] == 1) && (pr->tmp[2 * cj1 + 3] == -1))
            zj = -1;
          else
            goto cbrk;
          /****
               Handle the Independent Components in case Linear Number of Bounds
         are Created. Handling is simialr to Linear case above.
         *****/
          // acl = oo->acl;
          ...
          if ((pr->tmp[2 * cj1 + 2] == -1) && (pr->tmp[2 * cj1 + 3] == 1))
            ...
            goto cbrk;
          if ((pr->tmp[2 * cj2 + 2] == -1) && (pr->tmp[2 * cj2 + 3] == 1))
            zj = 1;
          else if ((pr->tmp[2 * cj2 + 2] == 1) && (pr->tmp[2 * cj2 + 3] == -1))
            zj = -1;
          else
            goto cbrk;
