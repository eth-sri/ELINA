...
opt_zones_t *opt_zones_meet(elina_manager_t *man, bool destructive,
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_MEET, 0);
  opt_zones_mat_t *oz;
...
array_comp_list_t *compute_finest_zones(opt_zones_mat_t *oz,
  ...
  while (cl != NULL) {
    ...
    for (i = 0; i < comp_size; i++) {
      ...
      for (j = 0; j < i; j++) {
        ...
        if (m[n * (i1 + 1) + (j1 + 1)] != INFINITY ||
            m[n * (j1 + 1) + i1 + 1] != INFINITY) {
          comp_list_t *cl = create_comp_list();
          insert_comp(cl, i1);
          insert_comp(cl, j1);
...
opt_zones_t *opt_zones_join(elina_manager_t *man, bool destructive,
  ...
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_JOIN, 0);
  ...
  if (!o1->closed && !o1->m) {
    if (!o2->closed && !o2->m) {
      ...
      r = opt_zones_set_mat(pr, o1, opt_zones_mat_copy(o2->m, o1->dim),
                            opt_zones_mat_copy(o2->closed, o1->dim),
                            destructive);
...
opt_zones_t *opt_zones_widening(elina_manager_t *man, opt_zones_t *o1,
  ...
  // fflush(stdout);
  opt_zones_internal_t *pr =
      opt_zones_init_from_manager(man, ELINA_FUNID_WIDENING, 0);
